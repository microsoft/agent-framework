...
internal sealed class TestRequestAgent(TestAgentRequestType requestType, int unpairedRequestCount, int pairedRequestCount, string? id, string? name) : AIAgent
{
    ...
    protected override ValueTask<AgentSession> CreateSessionCoreAsync(CancellationToken cancellationToken)
        => new(requestType switch
        {
            ...
            TestAgentRequestType.UserInputRequest => new TestRequestAgentSession<InputRequestContent, InputResponseContent>(),
    ...
    private sealed class FunctionApprovalStrategy : IRequestResponseStrategy<InputRequestContent, InputResponseContent>
    {
        public InputResponseContent CreatePairedResponse(InputRequestContent request)
        {
        ...
        public IEnumerable<(string, InputRequestContent)> CreateRequests(int count)
        {
            for (int i = 0; i < count; i++)
            {
                string id = Guid.NewGuid().ToString("N");
                InputRequestContent request = new ToolApprovalRequestContent(id, new FunctionCallContent(id, "TestFunction"));
                yield return (id, request);
            }
        }

        public void ProcessResponse(InputResponseContent response, TestRequestAgentSession<InputRequestContent, InputResponseContent> session)
        {
            if (session.UnservicedRequests.TryGetValue(response.RequestId, out InputRequestContent? request))
            {
                if (request is not ToolApprovalRequestContent approvalRequest)
                {
                    throw new InvalidOperationException($"Invalid request: Expecting {typeof(ToolApprovalResponseContent)}, got {request.GetType()}");
                ...
                approvalResponse.ToolCall.As<FunctionCallContent>().Should().Be(approvalRequest.ToolCall);
        ...
        where TRequest : AIContent
    {
        return request switch
        {
            ...
            InputRequestContent inputRequest => inputRequest.RequestId,
        ...
        where TRequest : AIContent
    {
        switch (requestType)
        {
            ...
            case TestAgentRequestType.UserInputRequest:
                if (!typeof(InputRequestContent).IsAssignableFrom(typeof(TRequest)))
                {
                    throw new ArgumentException($"Invalid request type: Expected {typeof(InputRequestContent)}, got {typeof(TRequest)}", nameof(requests));
                }

                return this.ValidateUnpairedRequests((IEnumerable<InputRequestContent>)requests, new FunctionApprovalStrategy());
            default:
    ...
    internal IEnumerable<ExternalResponse> ValidateUnpairedRequests(List<ExternalRequest> requests)
    {
        ...
        switch (requestType)
        {
            case TestAgentRequestType.FunctionCall:
                ...
                break;
            case TestAgentRequestType.UserInputRequest:
                responses = this.ValidateUnpairedRequests(requests.Select(AssertAndExtractRequestContent<InputRequestContent>)).ToList();
                break;
            default:
                throw new NotSupportedException($"Unknown AgentRequestType {requestType}");
        }
