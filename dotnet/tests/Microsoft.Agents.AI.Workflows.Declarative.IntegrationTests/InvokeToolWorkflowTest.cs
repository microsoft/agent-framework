...
public sealed class InvokeToolWorkflowTest(ITestOutputHelper output) : IntegrationTest(output)
{
        ...
        List<string> invokedFunctions)
    {
        ...
        foreach (ChatMessage message in toolRequest.AgentResponse.Messages)
        {
            ...
            {
                var toolName = approvalRequest.ToolCall switch
                {
                    FunctionCallContent fcc => fcc.Name,
                    McpServerToolCallContent mcc => mcc.Name,
                    _ => throw new InvalidOperationException("Unknown tool content type in approval request.")
                };
                this.Output.WriteLine($"APPROVAL REQUEST: {toolName}");
