// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;

namespace Microsoft.Agents.AI.Hosting.OpenAI.Responses.Generated.Models
{
    /// <summary> Emitted when a partial code snippet is streamed by the code interpreter. </summary>
    internal partial class ResponseCodeInterpreterCallCodeDeltaEvent : ResponseStreamEvent
    {
        /// <summary> Initializes a new instance of <see cref="ResponseCodeInterpreterCallCodeDeltaEvent"/>. </summary>
        /// <param name="sequenceNumber"> The sequence number for this event. </param>
        /// <param name="outputIndex"> The index of the output item in the response for which the code is being streamed. </param>
        /// <param name="itemId"> The unique identifier of the code interpreter tool call item. </param>
        /// <param name="delta"> The partial code snippet being streamed by the code interpreter. </param>
        /// <exception cref="ArgumentNullException"> <paramref name="itemId"/> or <paramref name="delta"/> is null. </exception>
        internal ResponseCodeInterpreterCallCodeDeltaEvent(int sequenceNumber, int outputIndex, string itemId, string delta) : base(sequenceNumber)
        {
            Argument.AssertNotNull(itemId, nameof(itemId));
            Argument.AssertNotNull(delta, nameof(delta));

            Type = ResponseStreamEventType.ResponseCodeInterpreterCallCodeDelta;
            OutputIndex = outputIndex;
            ItemId = itemId;
            Delta = delta;
        }

        /// <summary> Initializes a new instance of <see cref="ResponseCodeInterpreterCallCodeDeltaEvent"/>. </summary>
        /// <param name="type"></param>
        /// <param name="sequenceNumber"> The sequence number for this event. </param>
        /// <param name="serializedAdditionalRawData"> Keeps track of any properties unknown to the library. </param>
        /// <param name="outputIndex"> The index of the output item in the response for which the code is being streamed. </param>
        /// <param name="itemId"> The unique identifier of the code interpreter tool call item. </param>
        /// <param name="delta"> The partial code snippet being streamed by the code interpreter. </param>
        internal ResponseCodeInterpreterCallCodeDeltaEvent(ResponseStreamEventType type, int sequenceNumber, IDictionary<string, BinaryData> serializedAdditionalRawData, int outputIndex, string itemId, string delta) : base(type, sequenceNumber, serializedAdditionalRawData)
        {
            OutputIndex = outputIndex;
            ItemId = itemId;
            Delta = delta;
        }

        /// <summary> Initializes a new instance of <see cref="ResponseCodeInterpreterCallCodeDeltaEvent"/> for deserialization. </summary>
        internal ResponseCodeInterpreterCallCodeDeltaEvent()
        {
        }

        /// <summary> The index of the output item in the response for which the code is being streamed. </summary>
        public int OutputIndex { get; }
        /// <summary> The unique identifier of the code interpreter tool call item. </summary>
        public string ItemId { get; }
        /// <summary> The partial code snippet being streamed by the code interpreter. </summary>
        public string Delta { get; }
    }
}
