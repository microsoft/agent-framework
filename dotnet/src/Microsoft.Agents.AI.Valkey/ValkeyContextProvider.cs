...
public sealed class ValkeyContextProvider : MessageAIContextProvider, IAsyncDisposable
{
    ...
    private readonly SemaphoreSlim _indexCreationGate = new(1, 1);
    ...
    protected override async ValueTask StoreAIContextAsync(InvokedContext context, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            this._logger?.LogInformation("ValkeyContextProvider: Stored messages.");
        }
        catch (OperationCanceledException)
        {
            throw;
    ...
    private async Task EnsureIndexAsync(CancellationToken cancellationToken)
    {
        ...
        await this._indexCreationGate.WaitAsync(cancellationToken).ConfigureAwait(false);
        try
        {
            lock (this._indexLock)
            {
        ...
        {
            this._indexCreationGate.Release();
        }
    }
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        this._disposed = true;
        this._indexCreationGate.Dispose();
