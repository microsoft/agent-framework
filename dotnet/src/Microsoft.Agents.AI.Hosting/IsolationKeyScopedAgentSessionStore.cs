...
/// provided by a <see cref="StoreIsolationKeyProvider"/>, ensuring that sessions are isolated
...
{
    private readonly StoreIsolationKeyProvider? _keyProvider;
    ...
    /// The <see cref="StoreIsolationKeyProvider"/> used to retrieve the isolation key for the current context.
    /// </param>
    ...
    public IsolationKeyScopedAgentSessionStore(
        ...
        StoreIsolationKeyProvider? keyProvider,
    ...
    private async ValueTask<string?> GetIsolationKeyAsync(CancellationToken cancellationToken)
    {
        string? key = this._keyProvider != null
                    ? await this._keyProvider.GetStoreIsolationKeyAsync(cancellationToken).ConfigureAwait(false)
                    : null;

        if (this._strict && key == null)
        {
            throw new InvalidOperationException("Store isolation key is required but was not provided by the configured StoreIsolationKeyProvider.");
        }
