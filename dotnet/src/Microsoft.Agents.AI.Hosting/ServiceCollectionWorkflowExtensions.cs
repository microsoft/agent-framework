...
/// <summary>
/// Provides extension methods for configuring AI workflows in a service collection.
/// </summary>
public static class ServiceCollectionWorkflowExtensions
{
    /// <summary>
    ...
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="services"/>, <paramref name="name"/>, or <paramref name="createWorkflowDelegate"/> is null.</exception>
    ...
    /// </exception>
    public static IHostedWorkflowBuilder AddWorkflow(this IServiceCollection services, string name, Func<IServiceProvider, string, Workflow> createWorkflowDelegate)
    {
        ...
        services.AddKeyedSingleton(name, (sp, key) =>
        {
            ...
            var workflow = createWorkflowDelegate(sp, keyString) ?? throw new InvalidOperationException($"The workflow factory did not return a valid {nameof(Workflow)} instance for key '{keyString}'.");
        ...
        return new HostedWorkflowBuilder(name, services);
    }
}
