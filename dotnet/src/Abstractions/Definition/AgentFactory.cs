...
public abstract class AgentFactory
{
    ...
    public async Task<Agent> CreateAsync(IServiceProvider serviceProvider, AgentDefinition agentDefinition, AgentCreationOptions? agentCreationOptions = null, CancellationToken cancellationToken = default)
    {
        Verify.NotNull(agentDefinition);

        var kernelAgent = await this.TryCreateAsync(serviceProvider, agentDefinition, agentCreationOptions, cancellationToken).ConfigureAwait(false);
    ...
    /// <param name="cancellationToken">Optional cancellation token.</param>
    /// <return>The created <see cref="Agent"/>, if null the agent type is not supported.</return>
    public abstract Task<Agent?> TryCreateAsync(IServiceProvider serviceProvider, AgentDefinition agentDefinition, AgentCreationOptions? agentCreationOptions = null, CancellationToken cancellationToken = default);
}
