...
public sealed class AggregatorKernelAgentFactory : AgentFactory
{
    ...
    /// <inheritdoc/>
    public override async Task<Agent?> TryCreateAsync(IServiceProvider serviceProvider, AgentDefinition agentDefinition, AgentCreationOptions? agentCreationOptions = null, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var kernelAgentFactory in this._kernelAgentFactories)
        {
            ...
            {
                var kernelAgent = await kernelAgentFactory.TryCreateAsync(serviceProvider, agentDefinition, agentCreationOptions, cancellationToken).ConfigureAwait(false);
