...
public static class A2AServerServiceCollectionExtensions
{
    ...
    /// lookup key — typically by calling <c>UseClaimsBasedStoreIsolation(...)</c> from
    /// <c>Microsoft.Agents.AI.Hosting.AspNetCore</c> (or by registering a custom
    /// <see cref="StoreIsolationKeyProvider"/>). When a <see cref="StoreIsolationKeyProvider"/>
    ...
    /// compose a principal dimension via <c>UseClaimsBasedStoreIsolation(...)</c> or
    /// a custom <see cref="StoreIsolationKeyProvider"/>).
    /// </remarks>
    ...
    {
        var isolationKeyProvider = serviceProvider.GetService<StoreIsolationKeyProvider>();
