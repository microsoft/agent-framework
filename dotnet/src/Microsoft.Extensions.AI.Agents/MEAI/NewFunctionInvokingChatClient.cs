        ...
        ref bool lastIterationHadConversationId)
    {
        ...
        if (response.ConversationId is not null)
        {
            ...
            else
            {
                ...
                // This is needed to allow the ApprovalGeneratingChatClient to work.
                // It adds all FunctionApprovalResponseContent that were provided by the caller back into
                // the downstream request messages so that they can be processed by the inner client.
                // The ones that have matching FunctionCallContent are filtered out, and the ones without matching
                // FunctionCallContent are converted to rejected FunctionCallContent.
                var functionApprovals = originalMessages.SelectMany(x => x.Contents).OfType<FunctionApprovalResponseContent>().Cast<AIContent>().ToList();
                if (functionApprovals.Count > 0)
                {
                    augmentedHistory.Add(new ChatMessage(ChatRole.User, functionApprovals));
                }
            }
