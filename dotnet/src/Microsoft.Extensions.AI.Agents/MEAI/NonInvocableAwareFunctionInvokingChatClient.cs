...
using Microsoft.Extensions.AI.Agents.MEAI;
...
/// </para>
/// </remarks>
public partial class NonInvocableAwareFunctionInvokingChatClient : DelegatingChatClient
{
    ...
    public NonInvocableAwareFunctionInvokingChatClient(IChatClient innerClient, ILoggerFactory? loggerFactory = null, IServiceProvider? functionInvocationServices = null)
        : base(innerClient)
    {
        _logger = (ILogger?)loggerFactory?.CreateLogger<NonInvocableAwareFunctionInvokingChatClient>() ?? NullLogger.Instance;
        ...
        IEnumerable<ChatMessage> messages, ChatOptions? options = null, CancellationToken cancellationToken = default)
    {
        ...
        NonInvocableAIFunction[] nonInvocableAIFunctions = [.. options?.Tools?.OfType<NonInvocableAIFunction>() ?? [], .. AdditionalTools?.OfType<NonInvocableAIFunction>() ?? []];
        HashSet<string> nonInvocableAIFunctionNames = new(nonInvocableAIFunctions.Select(f => f.Name));
        HashSet<string> alreadyInvokedFunctionCalls = new(originalMessages.SelectMany(m => m.Contents).OfType<FunctionResultContent>().Select(frc => frc.CallId));

        // Any function call work to do? If yes, ensure we're tracking that work in functionCallContents.
        bool requiresPreGetResponseFunctionInvocation =
            (options?.Tools is { Count: > 0 } || AdditionalTools is { Count: > 0 }) &&
            CopyFunctionCalls(originalMessages, nonInvocableAIFunctionNames, alreadyInvokedFunctionCalls, ref functionCallContents);

        if (requiresPreGetResponseFunctionInvocation)
        {
            // Add the responses from the function calls into the augmented history and also into the tracked
            // list of response messages.
            var modeAndMessages = await ProcessFunctionCallsAsync(originalMessages, options, functionCallContents!, iteration++, consecutiveErrorCount, isStreaming: false, cancellationToken);
            responseMessages = [.. modeAndMessages.MessagesAdded];
        ...
        for (; ; iteration++)
        {
            // Do pre-invocation function calls, that may be passed in.
            ...
            bool requiresFunctionInvocation =
                ...
                CopyFunctionCalls(response.Messages, nonInvocableAIFunctionNames, [], ref functionCallContents);
        ...
        IEnumerable<ChatMessage> messages, ChatOptions? options = null, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        for (int iteration = 0; ; iteration++)
        {
            ...
            await foreach (var update in base.GetStreamingResponseAsync(messages, options, cancellationToken))
            {
                ...
                _ = CopyFunctionCalls(update.Contents, nonInvocableAIFunctionNames, [], ref functionCallContents);
    ...
    private static bool CopyFunctionCalls(
        IList<ChatMessage> messages, HashSet<string> nonInvocableAIFunctionNames, HashSet<string> alreadyInvokedFunctionCalls, [NotNullWhen(true)] ref List<FunctionCallContent>? functionCalls)
    {
        bool any = false;
        int count = messages.Count;
        for (int i = 0; i < count; i++)
        {
            any |= CopyFunctionCalls(messages[i].Contents, nonInvocableAIFunctionNames, alreadyInvokedFunctionCalls, ref functionCalls);
        }
    ...
    private static bool CopyFunctionCalls(
        IList<AIContent> content, HashSet<string> nonInvocableAIFunctionNames, HashSet<string> alreadyInvokedFunctionCalls, [NotNullWhen(true)] ref List<FunctionCallContent>? functionCalls)
    {
        bool any = false;
        int count = content.Count;
        for (int i = 0; i < count; i++)
        {
            if (content[i] is FunctionCallContent functionCall && !nonInvocableAIFunctionNames.Contains(functionCall.Name) && !alreadyInvokedFunctionCalls.Contains(functionCall.CallId))
            {
