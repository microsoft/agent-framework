...
public static class InProcessExecution
{
    ...
    public static ValueTask<StreamingRun> OpenStreamingAsync(Workflow workflow, string? runId = null, CancellationToken cancellationToken = default)
        => Default.OpenStreamingAsync(workflow, runId, cancellationToken);

    /// <inheritdoc cref="IWorkflowExecutionEnvironment.RunStreamingAsync{TInput}(Workflow, TInput, string?, CancellationToken)"/>
    public static ValueTask<StreamingRun> RunStreamingAsync<TInput>(Workflow workflow, TInput input, string? runId = null, CancellationToken cancellationToken = default) where TInput : notnull
        => Default.RunStreamingAsync(workflow, input, runId, cancellationToken);

    /// <inheritdoc cref="IWorkflowExecutionEnvironment.OpenStreamingAsync(Workflow, string?, CancellationToken)"/>
    public static ValueTask<StreamingRun> OpenStreamingAsync(Workflow workflow, CheckpointManager checkpointManager, string? runId = null, CancellationToken cancellationToken = default)
        => Default.WithCheckpointing(checkpointManager).OpenStreamingAsync(workflow, runId, cancellationToken);

    /// <inheritdoc cref="IWorkflowExecutionEnvironment.RunStreamingAsync{TInput}(Workflow, TInput, string?, CancellationToken)"/>
    public static ValueTask<StreamingRun> RunStreamingAsync<TInput>(Workflow workflow, TInput input, CheckpointManager checkpointManager, string? runId = null, CancellationToken cancellationToken = default) where TInput : notnull
        => Default.WithCheckpointing(checkpointManager).RunStreamingAsync(workflow, input, runId, cancellationToken);
