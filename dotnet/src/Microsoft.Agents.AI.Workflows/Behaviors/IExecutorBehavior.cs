...
public sealed class ExecutorBehaviorContext
{
    ...
    /// A distinct instance is created for each executor invocation, so values written by an outer behavior are
    /// visible to inner behaviors and to code running after <c>await continuation()</c> within the same call,
    /// but do not flow to subsequent executor invocations.
