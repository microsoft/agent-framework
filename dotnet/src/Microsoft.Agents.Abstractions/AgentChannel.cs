...
public abstract class AgentChannel
{
    ...
    protected internal abstract Task ReceiveAsync(IEnumerable<ChatMessage> history, CancellationToken cancellationToken = default);
    ...
    /// <summary>
    /// Perform a discrete incremental interaction between a single <see cref="Agent"/>.
    /// </summary>
    ...
    /// </remarks>
    protected internal abstract IAsyncEnumerable<(bool IsVisible, ChatMessage Message)> InvokeAsync(
        Agent agent,
        CancellationToken cancellationToken = default);

    /// <summary>
    /// Perform a discrete incremental interaction between a single <see cref="Agent"/> with streaming results.
    /// </summary>
    /// <param name="agent">The agent actively interacting with the chat.</param>
    ...
    protected internal abstract IAsyncEnumerable<ChatResponseUpdate> InvokeStreamingAsync(
        Agent agent,
    ...
    /// <param name="cancellationToken">The <see cref="CancellationToken"/> to monitor for cancellation requests. The default is <see cref="CancellationToken.None"/>.</param>
    /// <returns>Asynchronous enumeration of messages.</returns>
    protected internal abstract IAsyncEnumerable<ChatMessage> GetHistoryAsync(CancellationToken cancellationToken = default);
}
...
{
    /// <summary>
    /// Process a discrete incremental interaction between a single <see cref="Agent"/>.
    /// </summary>
    /// <param name="agent">The agent actively interacting with the chat.</param>
    /// <param name="cancellationToken">The <see cref="CancellationToken"/> to monitor for cancellation requests. The default is <see cref="CancellationToken.None"/>.</param>
    /// <returns>Asynchronous enumeration of messages.</returns>
    ...
    /// <inheritdoc/>
    protected internal override IAsyncEnumerable<(bool IsVisible, ChatMessage Message)> InvokeAsync(
        Agent agent,
        CancellationToken cancellationToken = default)
    {
        if (agent.GetType() != typeof(TAgent))
        {
            throw new InvalidOperationException($"Invalid agent channel: {typeof(TAgent).Name}/{agent.GetType().Name}");
        }
    ...
    /// <inheritdoc/>
    protected internal override IAsyncEnumerable<ChatResponseUpdate> InvokeStreamingAsync(
        Agent agent,
        IList<ChatMessage> messages,
        CancellationToken cancellationToken = default)
    {
