...
public static class InProcessExecution
{
    ...
    internal static InProcessRunner<TResult> CreateRunner<TResult>(WorkflowWithOutput<TResult> workflow, CheckpointManager? checkpointManager, string? runId)
        => new(workflow, checkpointManager, runId);

    internal static InProcessRunner<TResult> CreateRunner<TInput, TResult>(WorkflowWithOutput<TInput, TResult> checkedWorkflow, CheckpointManager? checkpointManager, string? runId)
        where TInput : notnull
        => new(checkedWorkflow, checkpointManager, runId, [typeof(TInput)]);
    ...
    private static ValueTask<StreamingRun<TResult>> StreamAsync<TInput, TResult>(InProcessRunner<TResult> runner, TInput input, CancellationToken cancellation = default)
        where TInput : notnull
        => runner.StreamAsync(input, cancellation);

    private static ValueTask<Run<TResult>> RunAsync<TInput, TResult>(InProcessRunner<TResult> runner, TInput input, CancellationToken cancellation = default)
        where TInput : notnull
        => runner.RunAsync(input, cancellation);

    private static async ValueTask<Checkpointed<StreamingRun<TResult>>> StreamCheckpointedAsync<TInput, TResult>(InProcessRunner<TResult> runner, TInput input, CancellationToken cancellation = default)
        where TInput : notnull
    {
        StreamingRun<TResult> run = await StreamAsync(runner, input, cancellation).ConfigureAwait(false);
        await runner.CheckpointAsync().ConfigureAwait(false);
        return new(run, runner);
    }

    private static async ValueTask<Checkpointed<Run<TResult>>> RunCheckpointedAsync<TInput, TResult>(InProcessRunner<TResult> runner, TInput input, CancellationToken cancellation = default)
        where TInput : notnull
    {
        Run<TResult> run = await RunAsync(runner, input, cancellation).ConfigureAwait(false);
        await runner.CheckpointAsync().ConfigureAwait(false);
        return new(run, runner);
    }

    private static async ValueTask<Checkpointed<StreamingRun<TResult>>> ResumeStreamCheckpointedAsync<TResult>(InProcessRunner<TResult> runner, CheckpointInfo fromCheckpoint, CancellationToken cancellation = default)
    {
        StreamingRun<TResult> run = await runner.ResumeStreamAsync(fromCheckpoint, cancellation).ConfigureAwait(false);
        return new(run, runner);
    }

    private static async ValueTask<Checkpointed<Run<TResult>>> ResumeRunCheckpointedAsync<TResult>(InProcessRunner<TResult> runner, CheckpointInfo fromCheckpoint, CancellationToken cancellation = default)
    {
        Run<TResult> run = await runner.ResumeAsync(fromCheckpoint, cancellation).ConfigureAwait(false);
        return new(run, runner);
    }
    ...
    public static ValueTask<StreamingRun<TResult>> StreamAsync<TInput, TResult>(
        WorkflowWithOutput<TResult> workflow,
        TInput input,
        string? runId = null,
        CancellationToken cancellation = default) where TInput : notnull
    {
        InProcessRunner<TResult> runner = CreateRunner(workflow, checkpointManager: null, runId);
        return StreamAsync(runner, (object)input, cancellation);
    }
    ...
    /// <returns>A <see cref="StreamingRun{TResult}"/> that provides access to the results of the streaming
    /// run.</returns>
    public static ValueTask<Checkpointed<StreamingRun<TResult>>> StreamAsync<TInput, TResult>(
        WorkflowWithOutput<TResult> workflow,
        TInput input,
        CheckpointManager checkpointManager,
        string? runId = null,
        CancellationToken cancellation = default) where TInput : notnull
    {
        InProcessRunner<TResult> runner = CreateRunner(workflow, checkpointManager, runId);
        return StreamCheckpointedAsync(runner, (object)input, cancellation);
    }
    ...
    /// <returns>A <see cref="StreamingRun{TResult}"/> that provides access to the results of the streaming
    /// run.</returns>
    public static ValueTask<Checkpointed<StreamingRun<TResult>>> ResumeStreamAsync<TResult>(
        WorkflowWithOutput<TResult> workflow,
        CheckpointInfo fromCheckpoint,
        CheckpointManager checkpointManager,
        string? runId = null,
    ...
    /// <returns>A <see cref="StreamingRun{TResult}"/> that provides access to the results of the streaming
    /// run.</returns>
    public static ValueTask<Checkpointed<StreamingRun<TResult>>> ResumeStreamAsync<TInput, TResult>(
        WorkflowWithOutput<TInput, TResult> workflow,
        CheckpointInfo fromCheckpoint,
        CheckpointManager checkpointManager,
        string? runId = null,
    ...
    /// <returns>A <see cref="ValueTask{Run}"/> that represents the asynchronous operation. The result contains a <see
    /// cref="Run"/> for managing and interacting with the streaming run.</returns>
    public static ValueTask<Checkpointed<Run<TResult>>> RunAsync<TInput, TResult>(
        WorkflowWithOutput<TResult> workflow,
        TInput input,
        CheckpointManager checkpointManager,
        string? runId = null,
    ...
    /// <returns>A <see cref="ValueTask{Run}"/> that represents the asynchronous operation. The result contains a <see
    /// cref="Run"/> for managing and interacting with the streaming run.</returns>
    public static ValueTask<Checkpointed<Run<TResult>>> ResumeAsync<TResult>(
        WorkflowWithOutput<TResult> workflow,
        CheckpointInfo fromCheckpoint,
        CheckpointManager checkpointManager,
        string? runId = null,
    ...
    /// <returns>A <see cref="ValueTask{Run}"/> that represents the asynchronous operation. The result contains a <see
    /// cref="Run"/> for managing and interacting with the streaming run.</returns>
    public static ValueTask<Checkpointed<Run<TResult>>> ResumeAsync<TInput, TResult>(
        WorkflowWithOutput<TInput, TResult> workflow,
        CheckpointInfo fromCheckpoint,
        CheckpointManager checkpointManager,
        string? runId = null,
