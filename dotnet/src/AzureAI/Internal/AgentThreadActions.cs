...
internal static class AgentThreadActions
{
    ...
    */
    ...
    /// <param name="cancellationToken">The <see cref="CancellationToken"/> to monitor for cancellation requests. The default is <see cref="CancellationToken.None"/>.</param>
    public static async Task CreateMessageAsync(PersistentAgentsClient client, string threadId, ChatMessage message, CancellationToken cancellationToken)
    {
        if (message.Contents.Any(i => i is FunctionCallContent))
        {
            return;
        }
    ...
    public static async IAsyncEnumerable<ChatMessage> GetMessagesAsync(PersistentAgentsClient client, string threadId, ListSortOrder? messageOrder, [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        await foreach (PersistentThreadMessage message in messages.ConfigureAwait(false))
        {
            ...
            ChatMessage content = GenerateMessageContent(assistantName, message);
    ...
    /// <param name="cancellationToken">The <see cref="CancellationToken"/> to monitor for cancellation requests. The default is <see cref="CancellationToken.None"/>.</param>
    /// <returns>Asynchronous enumeration of messages.</returns>
    public static async IAsyncEnumerable<(bool IsVisible, ChatMessage Message)> InvokeAsync(
        AzureAIAgent agent,
        ...
        do
        {
            ...
            if (run.Status == RunStatus.RequiresAction)
            {
                ...
                if (functionCalls.Length > 0)
                {
                    ...
                    ChatMessage functionCallMessage = GenerateFunctionCallContent(agent.GetName(), functionCalls);
            ...
            foreach (RunStep completedStep in completedStepsToProcess)
            {
                ...
                else if (completedStep.Type == RunStepType.MessageCreation)
                {
                    ...
                    {
                        ChatMessage content = GenerateMessageContent(agent.GetName(), message, completedStep, logger);
    ...
    /// </remarks>
    public static async IAsyncEnumerable<ChatResponseUpdate> InvokeStreamingAsync(
        AzureAIAgent agent,
        PersistentAgentsClient client,
        string threadId,
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        do
        {
            ...
            await foreach (StreamingUpdate update in asyncUpdates.ConfigureAwait(false))
            {
                ...
                {
                    ChatResponseUpdate? toolContent = GenerateStreamingCodeInterpreterContent(agent.GetName(), detailsUpdate);
                    if (toolContent != null)
                    {
                    ...
                    {
                        yield return
                            new ChatResponseUpdate(ChatRole.Assistant, null)
                            {
            ...
            if (stepsToProcess.Count > 0)
            {
                ...
                foreach (RunStep step in stepsToProcess)
                {
                    if (step.StepDetails is RunStepMessageCreationDetails messageDetails)
                    {
                        ...
                        if (message != null)
                        {
                            ChatMessage content = GenerateMessageContent(agent.GetName(), message, step, logger);
    ...
    */
    #region private
    private static ChatMessage GenerateMessageContent(string? assistantName, PersistentThreadMessage message, RunStep? completedStep = null, ILogger? logger = null)
    {
        ChatRole role = new(message.Role.ToString());
        ...
        foreach (MessageContent itemContent in message.ContentItems)
        {
            ...
            {
                content.Contents.Add(new TextContent(textContent.Text));

                /* TODO: Add support for file references
                foreach (MessageTextAnnotation annotation in textContent.Annotations)
                {
                    ...
                    {
                        content.Contents.Add(annotationItem);
                    }
                    else
                    {
            ...
            {
                content.Contents.Add(new FileReferenceContent(imageContent.FileId));
            }
            */
        }
    ...
    private static ChatResponseUpdate GenerateStreamingMessageContent(string? assistantName, ThreadRun run, MessageContentUpdate update, ILogger? logger)
    {
        ChatResponseUpdate content =
            new(ChatRole.Assistant, content: null)
            {
                AuthorName = assistantName,
            };
        ...
        {
            content.Contents.Add(new TextContent(update.Text));
        }
        ...
        {
            content.Contents.Add(new StreamingFileReferenceContent(update.ImageFileId));
        }
        // Process annotations
    ...
    private static ChatResponseUpdate? GenerateStreamingCodeInterpreterContent(string? assistantName, RunStepDetailsUpdate update)
    {
        ChatResponseUpdate content =
            new(ChatRole.Assistant, content: null)
            {
        ...
        {
            content.Contents.Add(new TextContent(update.CodeInterpreterInput));
            content.AdditionalProperties = new AdditionalPropertiesDictionary { { AzureAIAgent.CodeInterpreterMetadataKey, true } };
        }
        ...
        if ((update.CodeInterpreterOutputs?.Count ?? 0) > 0)
        {
            foreach (RunStepDeltaCodeInterpreterOutput output in update.CodeInterpreterOutputs!)
            {
                ...
                {
                    content.Contents.Add(new StreamingFileReferenceContent(imageOutput.Image.FileId));
                }
            }
        }
        */

        return content.Contents.Count > 0 ? content : null;
    }
    ...
    */
    ...
    private static ChatMessage GenerateCodeInterpreterContent(string agentName, string pythonCode, RunStep completedStep)
    {
        AdditionalPropertiesDictionary metadata = GenerateToolCallMetadata(completedStep);
    ...
    private static ChatMessage GenerateFunctionCallContent(string agentName, IList<FunctionCallContent> functionCalls)
    {
        ChatMessage functionCallContent = new(ChatRole.Assistant, content: null)
        {
            AuthorName = agentName
        };
    ...
    private static ChatMessage GenerateFunctionResultContent(string agentName, IEnumerable<FunctionResultContent> functionResults, RunStep completedStep)
    {
        ChatMessage functionResultContent = new(ChatRole.Tool, content: null)
        {
            AuthorName = agentName,
    ...
    */
    ...
    private static AdditionalPropertiesDictionary GenerateToolCallMetadata(RunStep completedStep)
    {
        return new()
            {
