...
public sealed class AgentFileSkillScript : AgentSkillScript
{
    ...
    /// <param name="skillDirectoryFullPath">The trusted skill directory containing the script.</param>
    ...
    internal AgentFileSkillScript(string name, string fullPath, string skillDirectoryFullPath, AgentFileSkillScriptRunner? runner = null)
        : base(name)
    {
    ...
    public override async Task<object?> RunAsync(AgentSkill skill, JsonElement? arguments, IServiceProvider? serviceProvider, CancellationToken cancellationToken = default)
    {
        ...
        AgentFileSkillPathValidator.ValidateForUse(this.FullPath, this._skillDirectoryFullPath, "Script", this.Name);
