...
public sealed class AggregateAgentSkillsSource : AgentSkillsSource
{
    private readonly IReadOnlyList<AgentSkillsSource> _sources;

    /// <summary>
    /// Initializes a new instance of the <see cref="AggregateAgentSkillsSource"/> class.
    /// </summary>
    /// <param name="sources">The child sources to aggregate.</param>
    public AggregateAgentSkillsSource(IEnumerable<AgentSkillsSource> sources)
    {
        _ = Throw.IfNull(sources);
        this._sources = new List<AgentSkillsSource>(sources);
    }
