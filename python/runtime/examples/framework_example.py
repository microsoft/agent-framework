...
async def main():
    ...
    try:
        from agent_framework import AgentRunResponse, ChatClientAgent, ChatMessage
    ...
    if framework_available:
        try:
                ...
                async def complete_chat_async(self, messages, **kwargs):
                    ...
                    if last_message:
                        ...
                        if hasattr(last_message, "contents") and last_message.contents:
                            user_text = (
                                last_message.contents[0].text
                                if hasattr(last_message.contents[0], "text")
                                else str(last_message.contents[0])
                            )
            ...
            runtime.register_actor_type("framework-1", lambda actor_id: AgentActor(framework_agent_1))

            runtime.register_actor_type("framework-2", lambda actor_id: AgentActor(framework_agent_2))
    ...
    try:
        ...
        response_handle = await client.send_request(
            actor_id=echo_id,
            method="run",
            params={"messages": [{"role": "user", "content": "Hello from simple runtime agent!"}]},
        )
        ...
        if framework_agents_registered:
            ...
            response_handle = await client.send_request(
                actor_id=fw_id_1,
                method="run",
                params={"messages": [{"role": "user", "content": "Hello from framework integration!"}]},
            )
            ...
            for i in range(3):
                agent_id = ActorId(
                    type_name="framework-1" if i % 2 == 0 else "framework-2", instance_id=f"concurrent-{i}"
                )

                task = client.send_request(
                    actor_id=agent_id,
                    method="run",
                    params={"messages": [{"role": "user", "content": f"Concurrent message {i}"}]},
                )
                tasks.append(task)
            ...
            # First message
            response_handle = await client.send_request(
                actor_id=conv_id, method="run", params={"messages": [{"role": "user", "content": "Start conversation"}]}
            )
            ...
            response_handle = await client.send_request(
                actor_id=conv_id,
                method="run",
                params={"messages": [{"role": "user", "content": "Continue conversation"}]},
            )
