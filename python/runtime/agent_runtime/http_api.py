...
# Create FastAPI app
app = FastAPI(
    title="Agent Runtime API", description="HTTP API for the Python Agent Runtime", version="0.1.0", lifespan=lifespan
)
...
async def root():
    """Root endpoint"""
    return {"message": "Agent Runtime API", "version": "0.1.0", "status": "running"}
...
async def run_agent(agent_name: str, request: AgentRunRequest):
    ...
    try:
        ...
        response_handle = await client.send_request(actor_id, "run", {"messages": messages_data})
...
async def health_check():
    ...
    return {"status": "healthy", "runtime_running": runtime is not None and runtime._running}
...
if __name__ == "__main__":
    ...
    uvicorn.run("agent_runtime.http_api:app", host="0.0.0.0", port=8000, log_level="info", reload=False)
