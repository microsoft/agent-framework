...
class HttpActorResponseHandle(ActorResponseHandle):
    ...
    async def get_response(self) -> ActorResponseMessage:
        ...
        for _ in range(max_retries):
            try:
                response = await self._client.get(f"/actors/{self._actor_id.type_name}/requests/{self._message_id}")
    ...
    async def send_request(
        ...
        async with httpx.AsyncClient(base_url=self._base_url, timeout=30.0) as client:
            try:
                ...
                response = await client.post(f"/agents/{actor_id.type_name}/run", json=payload)
                    ...
                    if content:
                        ...
                        for word in words:
                            ...
                            yield ActorResponseMessage(
                                ...
                                status=RequestStatus.PENDING,
                                data={"progress": {"contents": [{"text": current_text.strip()}], "role": "assistant"}},
                            )
