...
from agent_framework.exceptions import ContentAdditionException
...
class StreamingContentMixin(AgentBaseModel, ABC):
    """Mixin class for all streaming contents."""
    ...
    def _merge_items_lists(self, other_items: list[Any]) -> list[Any]:
        ...
        new_items_list: list[Any] = self.items.copy()  # type: ignore

        if new_items_list or other_items:
            for other_item in other_items:
                added = False
                for id, item in enumerate(new_items_list):  # type: ignore
                ...
                if not added:
                    ...
                    new_items_list.append(other_item)  # type: ignore

        return new_items_list  # type: ignore
    ...
    def _merge_inner_contents(self, other_inner_content: Any | list[Any]) -> list[Any]:
        ...
        if isinstance(self.inner_content, list):  # type: ignore
            new_inner_contents_list = self.inner_content.copy()  # type: ignore
        else:
            new_inner_contents_list = [self.inner_content]  # type: ignore

        other_inner_content = (  # type: ignore
            other_inner_content
        ...
        new_inner_contents_list.extend(other_inner_content)  # type: ignore

        return new_inner_contents_list  # type: ignore
