...
| 8 | [Hyperlight CodeAct](responses/06_hyperlight_codeact/) | An agent that runs Python in a Hyperlight WebAssembly sandbox via `HyperlightCodeActProvider`. The model only sees `execute_code` and reaches local tools through `call_tool(...)` inside the sandbox. **Note:** Hyperlight needs hypervisor (`/dev/kvm`/MSHV) access; the default Foundry hosted-agent runtime does not currently provide it — see the sample README. |
