# Multi-channel hosting — spec samples

Reference samples for the design in
[ADR 0026](../../../../docs/decisions/0026-hosting-channels.md) and
[SPEC-002](../../../../docs/specs/002-python-hosting-channels.md).
...
| [`foundry_hosted_agent/`](./foundry_hosted_agent) | One Foundry agent, **Responses + Invocations only** — the minimal shape that is **runtime-compatible with the Foundry Hosted Agents platform** (ADR 0026 §11). | Ships with `Dockerfile` + `agent.yaml` + `agent.manifest.yaml` + `azure.yaml` so the same image runs locally **or** as a Foundry Hosted Agent (`azd up`). |
...
sample uses `[tool.uv.sources]` to wire its channel dependencies to the
in-tree workspace packages so changes flow immediately.
