...
async def upload_video_file(
    ...
    try:
        ...
        if not storage_account_name:
            return (
                "Error: AZURE_STORAGE_ACCOUNT_NAME environment variable is not set. "
        ...
        async with AsyncDefaultAzureCredential() as credential:
            async with BlobServiceClient(account_url, credential=credential) as blob_service_client:
                ...
                try:
                    ...
                    return (
                        f"File uploaded successfully to {blob_url}\n\n"
...
async def download_and_upload_video(
    ...
    try:
        ...
        try:
            ...
            return (
                f"ERROR: Failed to download video from {public_video_url}\n"
            ...
            async with AsyncDefaultAzureCredential() as credential:
                async with BlobServiceClient(account_url, credential=credential) as blob_service_client:
                    ...
                    try:
                        ...
                        return (
                            f" Video downloaded and uploaded successfully!\n\n"
                        ...
                        return (
                            f" Video downloaded and uploaded to {blob_url}\n\n"
...
async def generate_sas_url(
    blob_url: Annotated[
        str,
        Field(description="Azure Blob Storage URL (e.g., https://account.blob.core.windows.net/container/video.mp4)"),
    ],
    ...
    try:
        ...
        if not parsed.scheme or not parsed.netloc:
            return (
                "ERROR: Invalid blob URL format. Expected format: https://account.blob.core.windows.net/container/blob"
            )
        ...
        if len(hostname_parts) < 3 or hostname_parts[1] != "blob":
            return "ERROR: Invalid Azure Blob Storage URL. Expected format: https://account.blob.core.windows.net/container/blob"
        ...
        if len(path_parts) != 2:
            return "ERROR: Invalid blob path. Expected format: /container/blob"
...
def start_video_translation(
    video_url: Annotated[
        str,
        Field(description="URL of the video file to translate (must be publicly accessible or Azure Blob Storage URL)"),
    ],
    ...
    try:
        ...
        client = VideoTranslationClient(
            api_version="2024-05-20-preview", credential=credential, token_provider=token_provider
        )
        ...
        if success:
            return (
                f" Translation started successfully! (Non-blocking operation)\n\n"
...
def delete_translation(translation_id: Annotated[str, Field(description="The translation ID to delete")]) -> str:
    """
...
def create_iteration_with_subtitle(
    ...
    webvtt_file_kind: Annotated[
        str, Field(description="Type of WebVTT file: 'SourceLocaleSubtitle', 'TargetLocaleSubtitle', or 'MetadataJson'")
    ],
) -> str:
    """
    ...
    try:
        ...
        if webvtt_file_kind not in kind_map:
            return (
                f"ERROR: Invalid WebVTT file kind: '{webvtt_file_kind}'\n\n"
        ...
        if success:
            return (
                f" Iteration started successfully! (Non-blocking operation)\n\n"
                f" Translation ID: {translation_id}\n"
