...
distinguish system-generated lifecycle events from user-generated data-plane events.
...
async def main():
    ...
    State (SYSTEM): IN_PROGRESS
    ExecutorInvokeEvent (SYSTEM): ExecutorInvokeEvent(executor_id=writer)
    ExecutorCompletedEvent (SYSTEM): ExecutorCompletedEvent(executor_id=writer)
    ExecutorInvokeEvent (SYSTEM): ExecutorInvokeEvent(executor_id=reviewer)
    Workflow completed (USER): Drive the Future. Affordable Adventure, Electrified.
    ExecutorCompletedEvent (SYSTEM): ExecutorCompletedEvent(executor_id=reviewer)
