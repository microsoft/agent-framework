...
- Inside each sample: 
    ...
    - Copy `local.settings.json.template` to `local.settings.json`, then update `AZURE_OPENAI_ENDPOINT`, `AZURE_OPENAI_CHAT_DEPLOYMENT_NAME`, and `AZURE_OPENAI_API_KEY` so the Azure OpenAI SDK can authenticate. 
        - If you prefer managed auth, omit the key and configure your code to use `AzureCliCredential`, for example: `chat_client = AzureOpenAIChatClient(credential=AzureCliCredential())`. 
