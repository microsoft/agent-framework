...
The same tools are available directly on the ``AzureAIClient`` as well, so this wiring is not unique to the responses client.
...
async def main() -> None:
    ...
    query = (
        "List the tool categories available to you and when each category is useful."
    )
    print(f"User: {query}")
