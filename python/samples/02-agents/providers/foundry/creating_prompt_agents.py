...
from agent_framework.foundry import FoundryChatClient, deploy_as_prompt_agent, to_prompt_agent
...
2. Published in one step via ``deploy_as_prompt_agent(agent)``, which reuses
...
``to_prompt_agent`` and ``deploy_as_prompt_agent`` are experimental
...
async def main() -> None:
    ...
    # 2) Recommended: one-step deploy. `deploy_as_prompt_agent` reuses the FoundryChatClient's
    ...
    print(f"Prompt agent published via deploy_as_prompt_agent: {created.name} v{created.version}")
