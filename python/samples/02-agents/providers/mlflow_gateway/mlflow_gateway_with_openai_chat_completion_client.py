...
"""
MLflow AI Gateway with OpenAI Chat Completion Client Example

This sample demonstrates routing Agent Framework requests through the
MLflow AI Gateway using its unified OpenAI-compatible Chat Completions
endpoint.
...
This sample uses ``OpenAIChatCompletionClient`` (Chat Completions API)
rather than ``OpenAIChatClient`` (Responses API) because the unified
``/gateway/mlflow/v1`` endpoint speaks Chat Completions, which works
regardless of the provider that backs the gateway endpoint.
...
- MLFLOW_GATEWAY_ENDPOINT: Base URL for the gateway's Chat Completions
  endpoint (e.g., "http://localhost:5000/gateway/mlflow/v1/")
...
async def main() -> None:
    print("=== MLflow AI Gateway with OpenAI Chat Completion Client Agent Example ===")
