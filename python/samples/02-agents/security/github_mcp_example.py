...
def _print_context_label(config: SecureAgentConfig) -> None:
    ...
    """
    variable_store = config.get_variable_store()
    context_label = variable_store.get_context_label()
    print(
        "\n[CONTEXT LABEL] "
...
async def _enter_secure_mcp(stack: AsyncExitStack, github_pat: str) -> SecureMCPToolProxy:
...
async def run_cli_async(*, attack_mode: bool) -> None:
    ...
    try:
        async with AsyncExitStack() as stack:
            secure_github = await _enter_secure_mcp(stack, github_pat)
...
async def run_devui_async() -> None:
...
def main() -> None:
    ...
    try:
        if mode == "cli":
            asyncio.run(run_cli_async(attack_mode=attack))
        else:
            asyncio.run(run_devui_async())
