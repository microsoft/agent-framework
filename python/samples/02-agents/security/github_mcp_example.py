...
def _build_foundry_clients(endpoint: str) -> tuple[FoundryChatClient, FoundryChatClient]:
    """Create the main agent + quarantine Foundry chat clients."""
    ...
    return main_client, quarantine_client


async def _enter_secure_mcp(
    stack: AsyncExitStack, github_pat: str, *, debug: bool = False
) -> SecureMCPToolProxy:
    """Open the HTTP client and SecureMCPToolProxy and register cleanup on ``stack``.

    Returns the connected proxy so callers can read ``proxy.tools``.
    """
    ...
    secure_github = await stack.enter_async_context(SecureMCPToolProxy(mcp_tool=github_mcp))
    ...
    return secure_github


def _build_secure_config(quarantine_client: FoundryChatClient) -> SecureAgentConfig:
    """Build the SecureAgentConfig shared by CLI and DevUI modes."""
    return SecureAgentConfig(
...
async def run_cli_async(*, attack_mode: bool, debug: bool = False) -> None:
    ...
    main_client, quarantine_client = _build_foundry_clients(endpoint)

    try:
        async with AsyncExitStack() as stack:
            secure_github = await _enter_secure_mcp(stack, github_pat, debug=debug)
            config = _build_secure_config(quarantine_client)
...
async def run_devui_async(*, debug: bool = False) -> None:
    ...
    async with AsyncExitStack() as stack:
        ...
        host = "127.0.0.1"
        port = 8090
        auth_token = get_devui_auth_token()
