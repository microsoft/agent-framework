...
The `simulate_service_stored_history.py` sample uses `FoundryChatClient`, so set the usual Foundry settings first and sign in with the Azure CLI:
...
az login
uv run samples/02-agents/chat_client/simulate_service_stored_history.py
```
