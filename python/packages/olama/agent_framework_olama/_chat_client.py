...
class OlamaSettings(AFBaseSettings):
    """Olama settings."""
...
class OlamaChatClient(BaseChatClient):
    client: AsyncClient
    chat_model_id: str
    ...
    def __init__(
        ...
        """Initialize an Olama Chat client.
        ...
        """
        try:
            olama_settings = OlamaSettings(host=host, chat_model_id=chat_model_id)
        except ValidationError as ex:
            raise ServiceInitializationError("Failed to create Olama settings.", ex) from ex

        client = client or AsyncClient(host=olama_settings.host)

        super().__init__(
            client=client,  # type: ignore[reportCallIssue]
            chat_model_id=olama_settings.chat_model_id,  # type: ignore[reportCallIssue]
        )
    ...
    def _prepare_chat_history_for_request(self, messages: MutableSequence[ChatMessage]) -> list[Message]:
        list_of_list = [self._olama_chat_message_parser(msg) for msg in messages]
        # Flatten the list of lists into a single list
        return list(chain.from_iterable(list_of_list))

    def _olama_chat_message_parser(self, message: ChatMessage) -> list[Message]:
        messages: list[Message] = []
        for content in message.contents:
            if isinstance(content, TextContent):
                messages.append(Message(role=message.role.value, content=content.text))
        return messages
