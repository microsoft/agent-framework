...
async def test_analyze_pdf_binary() -> None:
    ...
    # Use a minimal test PDF (or a real one from test_data/)
    test_data_dir = Path(__file__).parent / "test_data"
    pdf_files = list(test_data_dir.glob("*.pdf")) if test_data_dir.exists() else []

    if not pdf_files:
        pytest.skip("No test PDF files found in tests/cu/test_data/")
...
async def test_before_run_e2e() -> None:
    ...
    async with DefaultAzureCredential() as credential:
        cu = ContentUnderstandingContextProvider(
            ...
            max_wait=30.0,  # generous for integration test
        )
        async with cu:
            msg = Message(
                role="user",
                contents=[
                    ...
                    Content.from_data(
                        pdf_bytes,
                        "application/pdf",
                        additional_properties={"filename": pdf_files[0].name},
                    ),
                ],
            )
