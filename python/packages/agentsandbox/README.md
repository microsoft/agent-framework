...
1. `await sandbox.files.write("/app/_agent_sandbox_exec.py", code)` — ships the source as a file (sidesteps shell quoting; the model gets real tracebacks with file/line info).
2. `await sandbox.commands.run("python3 -u _agent_sandbox_exec.py")` via the Pod's `/execute` endpoint.
