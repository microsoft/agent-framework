...
def test_convert_tau2_tool_to_ai_function_basic():
    ...
    # Create a mock tau2 tool
    mock_tool = Mock(spec=Tool)
    mock_tool.name = "test_tool"
    mock_tool._get_description.return_value = "A test tool"

    # Create a mock params BaseModel
    class TestParams(BaseModel):
        param1: str
        param2: int = 42

    mock_tool.params = TestParams
    ...
    # Convert the tool
    ai_function = convert_tau2_tool_to_ai_function(mock_tool)

    # Verify the conversion
    assert isinstance(ai_function, AIFunction)
    assert ai_function.name == "test_tool"
    assert ai_function.description == "A test tool"
    assert ai_function.input_model == TestParams

    # Test calling the wrapped function
    result = ai_function.func(param1="test", param2=100)
    assert result == expected_result
    mock_tool.assert_called_once_with(param1="test", param2=100)


def test_convert_tau2_tool_to_ai_function_with_pydantic_result():
    """Test conversion when tau2 tool returns a Pydantic model."""
    mock_tool = Mock(spec=Tool)
    mock_tool.name = "pydantic_tool"
    mock_tool._get_description.return_value = "Tool returning Pydantic model"
    ...
    mock_tool.params = TestParams
    original_result = TestResult(output="test output", status=True)
    mock_tool.return_value = original_result

    ai_function = convert_tau2_tool_to_ai_function(mock_tool)
    result = cast(Any, ai_function.func(input_param="test"))
