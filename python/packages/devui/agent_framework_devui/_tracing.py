...
from ._models import DebugStreamEvent, TraceSpan
...
class StreamingTraceExporter(SpanExporter):
        ...
        try:
            ...
            for span in spans:
                ...
                if not thread_id:
                    ...
                    logger.warning(
                        f"No thread_id found in span {span.name if hasattr(span, 'name') else 'unknown'}, using fallback"
                    )
                ...
                event = DebugStreamEvent(
                    type="trace_span", trace_span=trace_span, timestamp=datetime.now().isoformat(), thread_id=thread_id
                )
    ...
    def _convert_to_trace_span(self, span: Any) -> TraceSpan:
        ...
        raw_span = {
            ...
            if hasattr(span, "status") and span.status.description
            else None,
            ...
            if hasattr(span, "resource") and span.resource.attributes
            else {},
            "trace_state": str(span.context.trace_state)
            if hasattr(span.context, "trace_state") and span.context.trace_state
            else None,
