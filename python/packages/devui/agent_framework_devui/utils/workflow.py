...
def extract_workflow_input_info(workflow: "Workflow") -> Dict[str, Any]:
    ...
    try:
        ...
        if (
            input_schema.get("type") in ["string", "integer", "number", "boolean", "array"]
...
def generate_schema(type_annotation: Any) -> Dict[str, Any]:
    ...
    return {"type": "object", "description": f"Complex type: {get_type_name(type_annotation)}"}
...
def _enhance_schema_with_function_signature(
    executor: Any, input_type: Any, base_schema: Dict[str, Any]
