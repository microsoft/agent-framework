...
class ProcessingRequest(BaseModel):
    ...
    enable_schema_validation: bool = Field(
        description="Enable schema validation checks",
        default=True
    )
    ...
    transformations: List[Literal["normalize", "enrich", "aggregate"]] = Field(
        ...
        default=["normalize", "enrich"]
    )
    
    # Optional description
    description: Optional[str] = Field(
        description="Optional description of the processing request",
        default=None
    )
    
    # Test failure scenarios
    force_validation_failure: bool = Field(
        description="Force validation failure for testing (demo purposes)",
        default=False
    )
    
    force_transformation_failure: bool = Field(
        description="Force transformation failure for testing (demo purposes)",
        default=False
    )
...
class DataIngestion(Executor):
    ...
    async def ingest_data(self, request: ProcessingRequest, ctx: WorkflowContext[DataBatch]) -> None:
        ...
        print(f"🔄 [DataIngestion] Starting data ingestion")
...
class DataQualityValidator(Executor):
    ...
    async def validate_quality(self, batch: DataBatch, ctx: WorkflowContext[ValidationReport]) -> None:
        ...
        result = ValidationResult.VALID if issues <= 1 else (
            ValidationResult.WARNING if issues <= 3 else ValidationResult.ERROR
        )
...
class ValidationAggregator(Executor):
    ...
    async def aggregate_validations(self, reports: List[ValidationReport], ctx: WorkflowContext[DataBatch]) -> None:
        ...
        print(f"📊 [ValidationAggregator] Validation Summary:")
        ...
        if should_fail:
            ...
            if quality_score < request.quality_threshold:
                failure_reason.append(f"quality score {quality_score:.2f} below threshold {request.quality_threshold:.2f}")
