...
from devui.execution import ExecutionEngine
...
class WorkflowInspector:
    ...
    async def run_and_capture(self, workflow, input_message: str, workflow_name: str = "workflow") -> List[Dict[str, Any]]:
    ...
    def _serialize_debug_event(self, debug_event: DebugStreamEvent) -> Dict[str, Any]:
        ...
        if debug_event.update:
            try:
                ...
                event_data["update"] = f"Serialization error: {str(e)}"
        ...
        if debug_event.trace_span:
            try:
                ...
                event_data["trace_span"] = f"Serialization error: {str(e)}"
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "execution_flow": [
                f"{we['executor_id']}: {we['event_type']}" 
...
async def main():
    ...
    print(f"💡 Usage: python inspector.py [input_message] [workflow_type]")
    print(f"   - workflow_type: 'simple', 'complex', or 'both' (default: 'both')")
