...
def ad_token_provider() -> str:
    ...
    token = credential.get_token(DEFAULT_AZURE_TOKEN_ENDPOINT)
    return token.token


# Create Azure OpenAI chat client
chat_client = AzureOpenAIResponsesClient(ad_token_provider=ad_token_provider)
...
workflow = (
    WorkflowBuilder(
        ...
        description="Multi-agent content creation workflow with quality-based routing (Writer → Reviewer → Editor/Publisher)",  # noqa: E501
    )
