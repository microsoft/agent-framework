...
class TestCosmosCheckpointStorageInit:
...
class TestCosmosCheckpointStorageContainerConfig:
    async def test_container_name_is_used(self, mock_cosmos_client: MagicMock) -> None:
        storage = CosmosCheckpointStorage(
            cosmos_client=mock_cosmos_client,
            database_name="db1",
...
class TestCosmosCheckpointStorageSave:
    async def test_save_upserts_document(self, mock_container: MagicMock) -> None:
        storage = CosmosCheckpointStorage(container_client=mock_container)
    ...
    async def test_save_returns_checkpoint_id(self, mock_container: MagicMock) -> None:
        storage = CosmosCheckpointStorage(container_client=mock_container)
...
class TestCosmosCheckpointStorageLoad:
    async def test_load_returns_checkpoint(self, mock_container: MagicMock) -> None:
    ...
    async def test_load_nonexistent_raises(self, mock_container: MagicMock) -> None:
    ...
    async def test_load_uses_cross_partition_query(self, mock_container: MagicMock) -> None:
        mock_container.query_items.return_value = _to_async_iter([])
...
class TestCosmosCheckpointStorageListCheckpoints:
    async def test_returns_checkpoints_for_workflow(self, mock_container: MagicMock) -> None:
    ...
    async def test_uses_partition_key(self, mock_container: MagicMock) -> None:
        mock_container.query_items.return_value = _to_async_iter([])
...
class TestCosmosCheckpointStorageDelete:
    async def test_delete_existing_returns_true(self, mock_container: MagicMock) -> None:
        mock_container.query_items.return_value = _to_async_iter([
    ...
    async def test_delete_nonexistent_returns_false(self, mock_container: MagicMock) -> None:
        mock_container.query_items.return_value = _to_async_iter([])
    ...
    async def test_delete_cosmos_not_found_returns_false(self, mock_container: MagicMock) -> None:
        mock_container.query_items.return_value = _to_async_iter([
            {"id": "cp-del", "workflow_name": "test-workflow"},
        ])
...
class TestCosmosCheckpointStorageGetLatest:
    async def test_returns_latest_checkpoint(self, mock_container: MagicMock) -> None:
    ...
    async def test_returns_none_when_empty(self, mock_container: MagicMock) -> None:
        mock_container.query_items.return_value = _to_async_iter([])
    ...
    async def test_uses_order_by_desc_with_limit(self, mock_container: MagicMock) -> None:
        mock_container.query_items.return_value = _to_async_iter([])
...
class TestCosmosCheckpointStorageListIds:
    async def test_returns_checkpoint_ids(self, mock_container: MagicMock) -> None:
        mock_container.query_items.return_value = _to_async_iter([
            {"checkpoint_id": "cp-1"},
            {"checkpoint_id": "cp-2"},
        ])
...
class TestCosmosCheckpointStorageClose:
    ...
    async def test_async_context_manager_preserves_original_exception(
...
class TestCosmosCheckpointStorageSaveLoadRoundTrip:
    async def test_round_trip_preserves_data(self, mock_container: MagicMock) -> None:
        """Test that saving and loading a checkpoint preserves all data."""
        checkpoint = _make_checkpoint(
        ...
        # Capture the document that was saved
        ...
        # Simulate Cosmos returning the saved document with system properties
        returned_doc = {
            **saved_doc,
            "_rid": "abc",
            "_self": "dbs/abc/colls/def/docs/ghi",
