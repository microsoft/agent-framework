...
async def test_handoff_with_participant_factories():
    ...
    # Follow-up message
    events = await _drain(
        workflow.run(stream=True, responses={requests[-1].request_id: [Message(role="user", text="More details")]})
    )
    outputs = [ev for ev in events if ev.type == "output"]
    assert outputs
...
async def test_handoff_with_participant_factories_and_add_handoff():
    ...
    events = await _drain(
        workflow.run(stream=True, responses={requests[-1].request_id: [Message(role="user", text="Need escalation")]})
    )
    requests = [ev for ev in events if ev.type == "request_info"]
    assert requests
...
async def test_handoff_participant_factories_with_checkpointing():
    ...
    events = await _drain(
        workflow.run(stream=True, responses={requests[-1].request_id: [Message(role="user", text="follow up")]})
    )
    outputs = [ev for ev in events if ev.type == "output"]
