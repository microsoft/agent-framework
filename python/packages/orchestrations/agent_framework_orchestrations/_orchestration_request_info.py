...
from agent_framework._agents import AgentLike
...
def resolve_request_info_filter(agents: list[str | AgentLike] | None) -> set[str]:
    """Resolve a list of agent/executor references to a set of IDs for filtering.

    Args:
        agents: List of agent names (str), AgentLike instances, or Executor instances.
...
class AgentApprovalExecutor(WorkflowExecutor):
    ...
    def _build_workflow(self, agent: AgentLike) -> Workflow:
