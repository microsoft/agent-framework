...
def test_collect_approval_responses_follow_up_does_not_suppress_response() -> None:
    ...
    # Response is still included because no terminal result exists
    assert "a2" in collected
    assert collected["a2"].type == "function_approval_response"


def test_collect_unanswered_requests_order_independent_result_first() -> None:
    """Unanswered check is order-independent."""
    ...
    # Terminal result before request in history
    messages = [
...
def test_remove_unanswered_batches_strips_resolved_local_responses() -> None:
    ...
    # Local response is stripped; answered request is preserved for context
