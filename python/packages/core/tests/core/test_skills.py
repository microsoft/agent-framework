...
# ---------------------------------------------------------------------------
# Tests: SkillFrontmatter construction and validation
# ---------------------------------------------------------------------------


class TestSkillFrontmatter:
    """Tests for SkillFrontmatter construction, defaults, and validation."""
    ...
    def test_all_fields(self) -> None:
        fm = SkillFrontmatter(
            ...
            metadata={"author": "alice", "version": "1.2.3"},
        )
        assert fm.name == "full-skill"
        assert fm.description == "Full skill."
        assert fm.license == "MIT"
        assert fm.compatibility == "agent-framework>=1.0"
        assert fm.allowed_tools == ["read_file", "write_file"]
        assert fm.metadata == {"author": "alice", "version": "1.2.3"}

    # ── compatibility ─────────────────────────────────────────────────────────

    def test_compatibility_max_length_accepted(self) -> None:
        long_compat = "x" * 500
        fm = SkillFrontmatter(name="s", description="d", compatibility=long_compat)
        assert fm.compatibility == long_compat

    def test_compatibility_too_long_raises(self) -> None:
        with pytest.raises(ValueError, match="compatibility"):
            SkillFrontmatter(name="s", description="d", compatibility="x" * 501)

    # ── name validation ───────────────────────────────────────────────────────

    def test_name_empty_raises(self) -> None:
        with pytest.raises(ValueError, match="cannot be empty"):
            SkillFrontmatter(name="", description="d")
    ...
    def test_name_uppercase_raises(self) -> None:
        with pytest.raises(ValueError, match="Invalid skill name"):
            SkillFrontmatter(name="My-Skill", description="d")

    def test_name_starts_with_hyphen_raises(self) -> None:
        with pytest.raises(ValueError, match="Invalid skill name"):
            SkillFrontmatter(name="-bad", description="d")

    def test_name_ends_with_hyphen_raises(self) -> None:
        with pytest.raises(ValueError, match="Invalid skill name"):
            SkillFrontmatter(name="bad-", description="d")

    def test_name_consecutive_hyphens_raises(self) -> None:
        with pytest.raises(ValueError, match="Invalid skill name"):
            SkillFrontmatter(name="bad--name", description="d")

    def test_name_too_long_raises(self) -> None:
        with pytest.raises(ValueError, match="Invalid skill name"):
            SkillFrontmatter(name="a" * 65, description="d")

    def test_name_max_length_accepted(self) -> None:
        fm = SkillFrontmatter(name="a" * 64, description="d")
        assert len(fm.name) == 64

    def test_name_with_numbers_accepted(self) -> None:
        fm = SkillFrontmatter(name="skill-v2", description="d")
        assert fm.name == "skill-v2"

    # ── description validation ────────────────────────────────────────────────

    def test_description_empty_raises(self) -> None:
        with pytest.raises(ValueError, match="cannot be empty"):
            SkillFrontmatter(name="s", description="")

    def test_description_whitespace_only_raises(self) -> None:
        with pytest.raises(ValueError, match="cannot be empty"):
            SkillFrontmatter(name="s", description="   ")

    def test_description_too_long_raises(self) -> None:
        with pytest.raises(ValueError, match="invalid description"):
            SkillFrontmatter(name="s", description="a" * 1025)

    def test_description_max_length_accepted(self) -> None:
        fm = SkillFrontmatter(name="s", description="a" * 1024)
