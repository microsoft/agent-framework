        ...
        with create_workflow_span(
            ...
            try:
                ...
                with _framework_event_origin():
                    started = WorkflowEvent.started()  # ruff: noqa: RUF070
                yield started
                with _framework_event_origin():
                    in_progress = WorkflowEvent.status(WorkflowRunState.IN_PROGRESS)  # ruff: noqa: RUF070
                yield in_progress
                ...
                async for event in self._runner.run_until_convergence():
                    ...
                    if event.type == "request_info" and not emitted_in_progress_pending:
                        ...
                        with _framework_event_origin():
                            pending_status = WorkflowEvent.status(WorkflowRunState.IN_PROGRESS_PENDING_REQUESTS)  # ruff: noqa: RUF070
                        yield pending_status
                # Workflow runs until idle - emit final status based on whether requests are pending
                if saw_request:
                    with _framework_event_origin():
                        terminal_status = WorkflowEvent.status(WorkflowRunState.IDLE_WITH_PENDING_REQUESTS)  # ruff: noqa: RUF070
                    yield terminal_status
                else:
                    with _framework_event_origin():
                        terminal_status = WorkflowEvent.status(WorkflowRunState.IDLE)  # ruff: noqa: RUF070
                    yield terminal_status
                ...
                with _framework_event_origin():
                    failed_event = WorkflowEvent.failed(details)  # ruff: noqa: RUF070
                yield failed_event
                with _framework_event_origin():
                    failed_status = WorkflowEvent.status(WorkflowRunState.FAILED)  # ruff: noqa: RUF070
                yield failed_status
                span.add_event(
