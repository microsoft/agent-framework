...
class TodoItem(SerializationMixin):
    ...
    @classmethod
    def from_dict(cls, raw_item: dict[str, Any]) -> TodoItem:
...
class TodoInput(SerializationMixin):
    ...
    @classmethod
    def from_dict(cls, raw_todo: dict[str, Any]) -> TodoInput:
...
def _coerce_todo_input(todo: TodoInput | dict[str, Any] | Any) -> TodoInput:
    ...
    if isinstance(todo, dict):
        return TodoInput.from_dict(todo)
...
class TodoSessionStore(TodoStore):
    ...
    def load_state(self, session: AgentSession, *, source_id: str) -> tuple[list[TodoItem], int]:
        ...
        items_payload = cast(list[Any], raw_items) if isinstance(raw_items, list) else []
...
class TodoFileStore(TodoStore):
    ...
    def load_state(self, session: AgentSession, *, source_id: str) -> tuple[list[TodoItem], int]:
        ...
        return [TodoItem.from_dict(cast(dict[str, Any], item)) for item in cast(list[Any], raw_items)], raw_next_id
