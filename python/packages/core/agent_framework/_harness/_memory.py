...
class MemoryTopicRecord:
    ...
    def from_dict(cls, raw_record: Mapping[str, Any]) -> MemoryTopicRecord:
        ...
        if not isinstance(memories, list) or not all(isinstance(value, str) for value in memories):
            raise ValueError("Memory topic record memories must be a list of strings.")
        if not isinstance(session_ids, list) or not all(isinstance(value, str) for value in session_ids):
            raise ValueError("Memory topic record session_ids must be a list of strings.")
        return cls(
            topic=topic,
            slug=slug,
            summary=summary,
            memories=cast(list[str], memories),
            updated_at=updated_at,
            session_ids=cast(list[str], session_ids),
        )
...
class MemoryFileStore(MemoryStore):
    ...
    def _decode_transcript_session_id(file_path: Path) -> str | None:
        ...
        if file_stem == FileHistoryProvider.DEFAULT_SESSION_FILE_STEM:
            return None
        encoded_prefix = FileHistoryProvider._ENCODED_SESSION_PREFIX
        if not file_stem.startswith(encoded_prefix):
            return file_stem
        encoded_value = file_stem[len(encoded_prefix) :]
...
class MemoryContextProvider(HistoryProvider):
    ...
    def _chat_client(
        ...
        *,
        ...
        return client if isinstance(client, SupportsChatGetResponse) else None
    ...
    async def _extract_memories(
        ...
        *,
        ...
        if not extracted_text:
            return []
        try:
            payload = json.loads(_extract_json_text(extracted_text))
        ...
        for raw_item in raw_items[: self.max_extractions]:
            if not isinstance(raw_item, Mapping):
                continue
            raw_topic = raw_item.get("topic")
            raw_memory = raw_item.get("memory")
    ...
    def _should_consolidate(self, state: Mapping[str, Any], *, now: datetime) -> bool:
        session_ids = state.get("sessions_since_consolidation")
        if not isinstance(session_ids, list) or len(session_ids) < self.consolidation_min_sessions:
            return False
    ...
    async def _consolidate_topic(
        ...
        *,
        ...
        if not consolidation_text:
            return record
        try:
            payload = json.loads(_extract_json_text(consolidation_text))
        ...
        if not isinstance(summary, str) or not isinstance(raw_memories, list):
            return record
        consolidated_memories = [memory for memory in raw_memories if isinstance(memory, str)]
        return MemoryTopicRecord(
            topic=record.topic,
            slug=record.slug,
            summary=summary,
