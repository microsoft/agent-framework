...
class LabelTrackingFunctionMiddleware(FunctionMiddleware):
    ...
    def _expand_variable_reference(self, value: Any) -> Any:
        """Expand bracketed variable references (e.g., ``[var_abc123]``) to stored content.

        This enables direct tool chaining where models pass variable placeholders as
        arguments to subsequent local/MCP tool calls.

        When a stored variable is a dict with a "response" key (e.g., from quarantined_llm),
        only the "response" value is extracted and returned, ensuring tools receive main
        content without metadata fields (security_label, metadata, quarantined, etc.).
        """
        if isinstance(value, str):
            # Use an unanchored pattern to find variable refs anywhere in the string
            # The global _BRACKETED_VAR_REF_RE has ^ and $ anchors for exact matching,
            # so we need a search-only pattern that finds embedded refs
            search_pattern = r"\[\s*(var_[0-9a-fA-F]+)\s*\]"
            matches = re.findall(search_pattern, value)

            if not matches:
                return value

            # If only one match and it's the entire string, expand directly
            if len(matches) == 1:
                match = _BRACKETED_VAR_REF_RE.match(value)
            ...
            # Multiple matches or embedded within text: substitute all occurrences in place
            def replace_var_ref(match_obj: Any) -> Any:
                ...
                try:
                    ...
                    return match_obj.group(0)  # Keep original if not found

            # Use unanchored pattern for substitution
            return re.sub(search_pattern, replace_var_ref, value)
    ...
    def _expand_variable_references_in_context(self, context: FunctionInvocationContext) -> None:
        ...
        if context.arguments is not None:
            ...
            if has_var_ref_before or has_var_ref_after:
                logger.info(f"Variable expansion for '{context.function.name}': had_ref_before={has_var_ref_before}, had_ref_after={has_var_ref_after}")
                if has_var_ref_before and not has_var_ref_after:
                    logger.info(f"  → Expanded variable references from: {args_before[:100]}... to: {args_after[:100]}...")
