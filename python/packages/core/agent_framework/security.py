...
def _label_from_mcp_meta(meta: Any) -> ContentLabel | None:
    ...
    if not isinstance(ifc, dict):
        return None
    if "$" in ifc: # gateway uses jsonpath format
        integ_raw = ifc.get("$").get("integrity")
        conf_raw = ifc.get("$").get("confidentiality")
    else:
...
class SecureMCPToolProxy:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            mcp_tool: An ``MCPTool`` instance to wrap.
            url: URL of a remote MCP server.
            headers: HTTP headers for *url* mode.
            name: Tool name for *url* mode.
            description: Tool description for *url* mode.
            default_integrity: Default integrity for tools without annotations.
            annotation_overrides: Per-tool-name label overrides.
            mark_write_tools_as_sinks: Whether to restrict write tools to PUBLIC.
