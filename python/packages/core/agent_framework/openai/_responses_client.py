...
__all__ = ["OpenAIBackend", "OpenAIResponsesClient", "OpenAIResponsesOptions"]
...
class OpenAIResponsesClient(
    ...
    """OpenAI Responses client with multi-backend support.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            api_version: Azure API version. Defaults to "preview" for Responses API.
            ...
            default_headers: Default headers for HTTP requests.
            client: Pre-configured AsyncAzureOpenAI client instance.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # For Azure Responses API, default api_version to "preview"
        if self._backend == "azure" and settings.api_version is None:
            settings.api_version = "preview"
        ...
        if self._backend == "openai":
            ...
            if not settings.responses_model_id:
                raise ServiceInitializationError(
        ...
        super().__init__(
            model_id=settings.responses_model_id,
            client=client,
    ...
    def _create_azure_client(
        ...
        # For Azure Responses API, we need special base_url handling
        azure_base_url = settings.azure_base_url
        if (
            not azure_base_url
            and settings.endpoint
            and hasattr(settings.endpoint, "host")
            and settings.endpoint.host
            and settings.endpoint.host.endswith(".openai.azure.com")
        ):
            # Azure Responses API requires /openai/v1/ path
            azure_base_url = urljoin(str(settings.endpoint), "/openai/v1/")  # type: ignore

        if azure_base_url:
            args["base_url"] = str(azure_base_url)
        if settings.endpoint and not azure_base_url:
            args["azure_endpoint"] = str(settings.endpoint)
        if settings.responses_model_id:
            args["azure_deployment"] = settings.responses_model_id
    ...
    def _check_model_presence(self, run_options: dict[str, Any]) -> None:
        """Check if model is present in run options."""
        if not run_options.get("model"):
            if not self.model_id:
                # Use appropriate error message based on backend
                if self._backend == "azure":
