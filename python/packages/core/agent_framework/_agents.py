...
__all__ = ["AgentLike", "BareAgent", "BaseAgent", "ChatAgent", "RawChatAgent"]
...
class AgentLike(Protocol):
    ...
    Examples:
        .. code-block:: python
            ...
            # No need to inherit from AgentLike or use any framework classes
            class CustomAgent:
                def __init__(self):
            ...
            assert isinstance(instance, AgentLike)
    """
...
class BaseAgent(SerializationMixin):
    ...
    Note:
        ...
        ``run()`` and other methods required by AgentLike.
    ...
    def as_tool(
        ...
        *,
        ...
        Raises:
            TypeError: If the agent does not implement AgentLike.
        ...
        """
        # Verify that self implements AgentLike
        if not isinstance(self, AgentLike):
            raise TypeError(f"Agent {self.__class__.__name__} must implement AgentLike to be used as a tool")
