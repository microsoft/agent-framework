...
- **`AgentLoopMiddleware`** - `AgentMiddleware` that re-runs an agent in a loop by calling `call_next()` repeatedly (the pipeline re-reads `context.messages` each time). One configurable class covers three patterns: the unbounded "Ralph" loop (no exit criteria), a user `should_continue` predicate, and a `judge_client` (a second chat client that decides whether the original request was answered; loops while it is *not*, using a `JudgeVerdict` structured-output response). Each pattern has a convenience classmethod factory that exposes only the relevant args and forwards to `__init__`: `AgentLoopMiddleware.ralph(...)`, `.with_predicate(should_continue, ...)`, and `.with_judge(judge_client, ...)`; use the full constructor for combined/advanced configs (e.g. approval handling). Supports both streaming and non-streaming runs (streaming returns the last iteration as the final response and handles `MiddlewareTermination` cleanly). Constructor args are all optional: `should_continue`, `judge_client`/`judge_instructions` (mutually exclusive with `should_continue`), `max_iterations` (`None`=unbounded; judge mode defaults to 5), `next_message` (defaults to a short "continue" nudge), and the approval args below. The `should_continue`/`next_message` callables are invoked with keyword args (`iteration`, `last_result`, `messages`, `original_messages`, `session`, `agent`); declare only what you need plus `**kwargs`.
  - **Approval handling** - `on_approval_request` resolves pending `user_input_request` content (e.g. tools with `approval_mode="always_require"`). It is called once per pending request as `on_approval_request(request=<Content>, **loop_kwargs)` and returns `bool` (approve/reject; function approvals only), a response `Content`, or `None` (no decision). Approvals are handled before the normal continue-criterion and are **exempt from `max_iterations`**; `max_approval_rounds` (default `None`) caps consecutive approval rounds. All-or-nothing per round: if any request returns `None`, nothing is submitted and the loop falls through to normal continuation. With a session only the user responses are sent; without one the original input + assistant request + user responses are re-sent.
  - **Ralph feedback tracking** - For Ralph-style loops, `record_feedback` captures a per-iteration progress entry (called with the loop kwargs; if it returns a truthy string the entry is appended, otherwise the agent's response text is used as the fallback entry). The accumulated log is exposed to every callback via the `progress` keyword (a per-iteration copy of prior entries) and, when `inject_progress=True` (default), injected into the next iteration's input as a `user` message (the full log without a session, only the latest entry with a session to avoid duplicating history). `fresh_context=True` restarts each iteration from the original task plus the progress log (Ralph's "fresh context per iteration"; warns if a session is attached). `is_complete` stops the loop early: a `str` is substring-matched against the last response text, or a callable is invoked with the loop kwargs and returns `bool`.
