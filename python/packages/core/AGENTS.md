...
- **Progressive MCP disclosure** (`use_progressive_disclosure`, `always_load`) - When enabled on any `MCPTool` subclass, the initial model-facing surface is loader tools (`list_mcp_tools` / `load_tool`, prefixed by `tool_name_prefix` when configured) plus allowed tools selected by `always_load`. `list_mcp_tools` only reports tools that pass `allowed_tools`; filtered tools are not listed or loadable. `load_tool` uses `FunctionInvocationContext.add_tools(...)` so the selected generated MCP `FunctionTool` becomes available on the next function-calling iteration and keeps its existing approval mode, argument filtering, header-provider runtime kwargs, result parsing, OTel, and task behavior. Invalid `always_load` entries are ignored like unmatched `allowed_tools` entries.
