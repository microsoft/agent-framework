...
async def test_reprovision_regenerates_name_for_autogenerated_defaults(
    ...
    Tenki can reject ``Sandbox.create(name=<terminated-sandbox-name>)`` with
    ``ALREADY_EXISTS`` if the workspace still holds the name; a fresh 8-hex
    suffix keeps the re-provision path healthy for the common case where the
    caller never passed an explicit ``sandbox_name``.
    """
    tool = TenkiExecuteCodeTool()  # no explicit sandbox_name → auto-generated
    ...
    # Two ``create`` calls, each with a different name; both carry the shared prefix.
    assert len(fake_sdk.create_calls) == 2
...
async def test_reprovision_preserves_explicit_sandbox_name(
    ...
    Regenerating would silently violate the caller's expressed intent; if Tenki
    rejects reuse we let the ``RuntimeError`` surface so the caller can act on it.
    """
...
async def test_cancelled_error_propagates(fake_sdk: _FakeSandboxFactory) -> None:
    """asyncio.CancelledError must propagate so higher-level workflows can stop promptly."""
    import asyncio
