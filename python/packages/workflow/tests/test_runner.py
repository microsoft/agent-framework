...
from agent_framework_workflow._runner_context import InProcRunnerContext, Message, RunnerContext
...
async def test_runner_run_until_convergence():
    ...
    async for event in runner.run_until_convergence():
        ...
        if isinstance(event, WorkflowCompletedEvent):
            result = event.data
            assert event.origin is WorkflowEventSource.USER
...
async def test_runner_emits_system_completion_for_agent_response_without_targets():
    ctx = InProcRunnerContext()
    ...
    assert completions
    assert all(e.origin is WorkflowEventSource.SYSTEM for e in completions)
