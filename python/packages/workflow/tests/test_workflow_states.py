...
async def test_executor_failed_and_workflow_failed_events_streaming():
    ...
    assert all(e.origin is WorkflowEventSource.SYSTEM for e in failed_events)
    status = [e for e in events if isinstance(e, WorkflowStatusEvent)]
    assert status and status[-1].state == WorkflowRunState.FAILED
    assert all(e.origin is WorkflowEventSource.SYSTEM for e in status)
...
async def test_executor_failed_event_emitted_on_direct_execute():
    ...
    assert failed
    assert all(e.origin is WorkflowEventSource.SYSTEM for e in failed)
...
async def test_started_and_completed_event_origins():
    ...
    assert started.origin is WorkflowEventSource.SYSTEM

    completed = next(e for e in events if isinstance(e, WorkflowCompletedEvent))
    assert completed.origin is WorkflowEventSource.USER
