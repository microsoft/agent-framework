...
def a2a_agent(mock_a2a_client: MockA2AClient) -> A2AAgent:
    ...
    agent = A2AAgent(name="Test Agent", id="test-agent", client=mock_a2a_client)  # type: ignore[arg-type]
...
def test_a2a_agent_initialization_with_client(mock_a2a_client: MockA2AClient) -> None:
    ...
    agent = A2AAgent(
        ...
        client=mock_a2a_client,  # type: ignore[arg-type]
    )
    agent.client = mock_a2a_client  # type: ignore[assignment]
...
async def test_run_with_task_response_no_artifacts(a2a_agent: A2AAgent, mock_a2a_client: MockA2AClient) -> None:
    ...
    assert isinstance(response, AgentRunResponse)
    assert len(response.messages) == 1  # Empty task creates empty message
    assert response.messages[0].contents == []  # But with no content
    assert response.messages[0].role == Role.ASSISTANT  # Assistant role
...
def test_a2a_parts_to_contents_conversion(a2a_agent: A2AAgent) -> None:
    ...
    agent = A2AAgent(name="Test Agent", client=MockA2AClient())  # type: ignore[arg-type]
    agent.client = MockA2AClient()  # type: ignore[assignment]
...
async def test_context_manager_cleanup() -> None:
    ...
    agent = A2AAgent(client=mock_a2a_client, http_client=mock_http_client)  # type: ignore[arg-type]
    agent.client = mock_a2a_client  # type: ignore[assignment]
    agent._http_client = mock_http_client  # type: ignore[assignment]
...
def test_chat_message_to_a2a_message_with_multiple_contents() -> None:
    ...
    agent = A2AAgent(client=mock_client)  # type: ignore[arg-type]
    agent.client = mock_client  # type: ignore[assignment]
