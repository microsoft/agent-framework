...
def test_init(lite_llm_unit_test_env: dict[str, str]) -> None:
    ...
    lite_llm_chat_completion = LiteLlmChatClient()

    assert lite_llm_chat_completion.model_id == lite_llm_unit_test_env["LITE_LLM_MODEL_ID"]
    assert isinstance(lite_llm_chat_completion, ChatClientProtocol)


def test_init_validation_fail() -> None:
    # Test successful initialization
    with pytest.raises(ServiceInitializationError):
        LiteLlmChatClient(api_key="34523", model_id={"test": "dict"})  # type: ignore


def test_init_model_id_constructor(lite_llm_unit_test_env: dict[str, str]) -> None:
    # Test successful initialization
    model_id = "azure_ai/test_model_id"
    lite_llm_chat_completion = LiteLlmChatClient(model_id=model_id)

    assert lite_llm_chat_completion.model_id == model_id
    assert isinstance(lite_llm_chat_completion, ChatClientProtocol)


def test_lite_llm_chat_client_instructions_sent_once(lite_llm_unit_test_env: dict[str, str]) -> None:
    """Ensure instructions are only included once for LiteLLM chat requests."""
...
def test_serialize(lite_llm_unit_test_env: dict[str, str]) -> None:
    ...
    lite_llm_chat_completion = LiteLlmChatClient.from_dict(settings)
    dumped_settings = lite_llm_chat_completion.to_dict()
...
def test_unsupported_tool_handling(lite_llm_unit_test_env: dict[str, str]) -> None:
...
async def test_lite_llm_chat_completion_response() -> None:
    """Test LiteLLM chat completion responses."""
    lite_llm_chat_client = LiteLlmChatClient()

    assert isinstance(lite_llm_chat_client, ChatClientProtocol)
    ...
    response = await lite_llm_chat_client.get_response(messages=messages)
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_chat_completion_response_tools() -> None:
    """Test LiteLLM chat completion responses."""
    lite_llm_chat_client = LiteLlmChatClient()
    ...
    # Test that the client can be used to get a response
    response = await lite_llm_chat_client.get_response(
        messages=messages,
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_chat_client_streaming() -> None:
    """Test LiteLLM chat completion responses."""
    lite_llm_chat_client = LiteLlmChatClient()
    ...
    # Test that the client can be used to get a response
    response = lite_llm_chat_client.get_streaming_response(messages=messages)
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_chat_client_streaming_tools() -> None:
    """Test LiteLLM chat completion responses."""
    lite_llm_chat_client = LiteLlmChatClient()
    ...
    # Test that the client can be used to get a response
    response = lite_llm_chat_client.get_streaming_response(
        messages=messages,
        tools=[get_story_text],
        tool_choice="auto",
    )
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_chat_client_agent_basic_run():
    """Test LiteLLM chat client agent basic run functionality with LiteLlmChatClient."""
    async with ChatAgent(
        chat_client=LiteLlmChatClient(model_id="azure_ai/gpt-4o-mini"),
    ) as agent:
        # Test basic run
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_chat_client_agent_basic_run_streaming():
    """Test LiteLLM chat client agent basic streaming functionality with LiteLlmChatClient."""
    async with ChatAgent(
        chat_client=LiteLlmChatClient(model_id="azure_ai/gpt-4o-mini"),
    ) as agent:
        # Test streaming run
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_chat_client_agent_thread_persistence():
    """Test LiteLLM chat client agent thread persistence across runs with LiteLlmChatClient."""
    async with ChatAgent(
        chat_client=LiteLlmChatClient(model_id="azure_ai/gpt-4o-mini"),
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_chat_client_agent_existing_thread():
    """Test LiteLLM chat client agent with existing thread to continue conversations across agent instances."""
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_chat_client_run_level_tool_isolation():
    """Test that run-level tools are isolated to specific runs and don't persist with LiteLLM Chat Client."""
...
def test_chat_response_content_order_text_before_tool_calls(lite_llm_unit_test_env: dict[str, str]):
...
def test_function_result_falsy_values_handling(lite_llm_unit_test_env: dict[str, str]):
    ...
    lite_llm_messages = client._openai_chat_message_parser(message_with_empty_list)
    assert len(lite_llm_messages) == 1
    assert lite_llm_messages[0]["content"] == "[]"  # Empty list should be JSON serialized
    ...
    lite_llm_messages = client._openai_chat_message_parser(message_with_empty_string)
    assert len(lite_llm_messages) == 1
    assert lite_llm_messages[0]["content"] == ""  # Empty string should be preserved
    ...
    lite_llm_messages = client._openai_chat_message_parser(message_with_false)
    assert len(lite_llm_messages) == 1
    assert lite_llm_messages[0]["content"] == "false"  # False should be JSON serialized


def test_function_result_exception_handling(lite_llm_unit_test_env: dict[str, str]):
    ...
    lite_llm_messages = client._openai_chat_message_parser(message_with_exception)
    assert len(lite_llm_messages) == 1
    assert lite_llm_messages[0]["content"] == "Error: Test error message"
    assert lite_llm_messages[0]["tool_call_id"] == "call-123"
...
def test_openai_content_parser_data_content_image(lite_llm_unit_test_env: dict[str, str]) -> None:
...
def test_openai_content_parser_document_file_mapping(lite_llm_unit_test_env: dict[str, str]) -> None:
