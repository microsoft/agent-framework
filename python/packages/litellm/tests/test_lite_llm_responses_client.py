...
skip_if_lite_llm_integration_tests_disabled = pytest.mark.skipif(
    os.getenv("RUN_INTEGRATION_TESTS", "false").lower() != "true",
    reason="Integration tests are disabled.",
)


async def delete_vector_store(client: LiteLlmResponsesClient, file_id: str, vector_store_id: str) -> None:
...
def test_init(lite_llm_unit_test_env: dict[str, str]) -> None:
    # Test successful initialization
    lite_llm_responses_client = LiteLlmResponsesClient()

    assert lite_llm_responses_client.model_id == lite_llm_unit_test_env["LITE_LLM_MODEL_ID"]
    assert isinstance(lite_llm_responses_client, ChatClientProtocol)


def test_init_model_id_constructor(lite_llm_unit_test_env: dict[str, str]) -> None:
    # Test successful initialization
    model_id = "test_model_id"
    lite_llm_responses_client = LiteLlmResponsesClient(model_id=model_id)

    assert lite_llm_responses_client.model_id == model_id
    assert isinstance(lite_llm_responses_client, ChatClientProtocol)


def test_lite_llm_responses_client_instructions_sent_once(lite_llm_unit_test_env: dict[str, str]) -> None:
    """Ensure instructions are only included once for LiteLLM Responses requests."""
...
@pytest.mark.parametrize("exclude_list", [["LITE_LLM_MODEL_ID"]], indirect=True)
def test_init_with_empty_model_id(lite_llm_unit_test_env: dict[str, str]) -> None:
...
def test_serialize(lite_llm_unit_test_env: dict[str, str]) -> None:
    default_headers = {"X-Unit-Test": "test-guid"}

    settings = {
        "model_id": lite_llm_unit_test_env["LITE_LLM_MODEL_ID"],
        "api_key": lite_llm_unit_test_env["OPENAI_API_KEY"],
        "default_headers": default_headers,
    }

    lite_llm_responses_client = LiteLlmResponsesClient.from_dict(settings)
    dumped_settings = lite_llm_responses_client.to_dict()
    assert dumped_settings["model_id"] == lite_llm_unit_test_env["LITE_LLM_MODEL_ID"]
...
def test_serialize_with_org_id(lite_llm_unit_test_env: dict[str, str]) -> None:
    settings = {
        "model_id": lite_llm_unit_test_env["LITE_LLM_MODEL_ID"],
        "api_key": lite_llm_unit_test_env["OPENAI_API_KEY"],
        "org_id": lite_llm_unit_test_env["OPENAI_ORG_ID"],
    }

    lite_llm_responses_client = LiteLlmResponsesClient.from_dict(settings)
    dumped_settings = lite_llm_responses_client.to_dict()
    assert dumped_settings["model_id"] == lite_llm_unit_test_env["LITE_LLM_MODEL_ID"]
    assert dumped_settings["org_id"] == lite_llm_unit_test_env["OPENAI_ORG_ID"]
    # Assert that the 'User-Agent' header is not present in the dumped_settings default headers
...
def test_get_response_with_invalid_input() -> None:
    ...
    client = LiteLlmResponsesClient(model_id="invalid-model", api_key="test-key")
...
def test_get_response_with_all_parameters() -> None:
    ...
    client = LiteLlmResponsesClient(model_id="test-model", api_key="test-key")
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
...
def test_tools_to_response_tools_with_raw_image_generation_lite_llm_responses_params() -> None:
    """Test raw image_generation tool with LiteLLM-specific parameters."""
    client = LiteLlmResponsesClient(model_id="test-model", api_key="test-key")

    # Test with LiteLLM-specific parameters
    tool = {
        "type": "image_generation",
        "size": "1024x1024",
        "model": "gpt-image-1",
    ...
    # Check LiteLLM-specific parameters are included
...
async def test_end_to_end_mcp_approval_flow(span_exporter) -> None:
    ...
    with patch.object(
        client, "make_responses_nonstreaming_request", side_effect=[mock_response1, mock_response2]
    ) as mock_create:
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_responses_client_response() -> None:
    """Test LiteLLM chat completion responses."""
    lite_llm_responses_client = LiteLlmResponsesClient()
    ...
    response = await lite_llm_responses_client.get_response(messages=messages)
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_responses_client_response_tools() -> None:
    """Test LiteLLM chat completion responses."""
    lite_llm_responses_client = LiteLlmResponsesClient()
    ...
    # Test that the client can be used to get a response
    response = await lite_llm_responses_client.get_response(
        messages=messages,
        tools=[get_weather],
        tool_choice="auto",
    )
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_responses_client_streaming() -> None:
    """Test LiteLLM chat completion responses."""
    lite_llm_responses_client = LiteLlmResponsesClient()
    ...
    response = await ChatResponse.from_chat_response_generator(
        lite_llm_responses_client.get_streaming_response(messages=messages)
    )
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_responses_client_streaming_tools() -> None:
    """Test LiteLLM chat completion responses."""
    lite_llm_responses_client = LiteLlmResponsesClient()
    ...
    # Test that the client can be used to get a response
    response = lite_llm_responses_client.get_streaming_response(
        messages=messages,
        tools=[get_weather],
        tool_choice="auto",
    )
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_responses_client_agent_basic_run():
    """Test LiteLLM Responses Client agent basic run functionality with LiteLlmResponsesClient."""
    agent = LiteLlmResponsesClient().create_agent(
        instructions="You are a helpful assistant.",
    )
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_responses_client_agent_basic_run_streaming():
    """Test LiteLLM Responses Client agent basic streaming functionality with LiteLlmResponsesClient."""
    async with ChatAgent(
        chat_client=LiteLlmResponsesClient(),
    ) as agent:
        # Test streaming run
        full_text = ""
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_responses_client_agent_thread_persistence():
    """Test LiteLLM Responses Client agent thread persistence across runs with LiteLlmResponsesClient."""
    async with ChatAgent(
        chat_client=LiteLlmResponsesClient(),
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_responses_client_agent_thread_storage_with_store_true():
    """Test LiteLLM Responses Client agent with store=True to verify service_thread_id is returned."""
    async with ChatAgent(
        chat_client=LiteLlmResponsesClient(),
        instructions="You are a helpful assistant.",
    ) as agent:
        ...
        # Run with store=True to store messages on LiteLLM side
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_responses_client_agent_existing_thread():
    """Test LiteLLM Responses Client agent with existing thread to continue conversations across agent instances."""
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_responses_client_agent_hosted_code_interpreter_tool():
    """Test LiteLLM Responses Client agent with HostedCodeInterpreterTool through LiteLlmResponsesClient."""
    async with ChatAgent(
        chat_client=LiteLlmResponsesClient(),
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_responses_client_agent_level_tool_persistence():
    """Test that agent-level tools persist across multiple runs with LiteLLM Responses Client."""
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_responses_client_run_level_tool_isolation():
    """Test that run-level tools are isolated to specific runs and don't persist with LiteLLM Responses Client."""
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_responses_client_agent_chat_options_run_level() -> None:
    """Integration test for comprehensive ChatOptions parameter coverage with LiteLLM Response Agent."""
    async with ChatAgent(
        chat_client=LiteLlmResponsesClient(),
        instructions="You are a helpful assistant.",
    ) as agent:
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_responses_client_agent_chat_options_agent_level() -> None:
    """Integration test for comprehensive ChatOptions parameter coverage with LiteLLM Response Agent."""
    async with ChatAgent(
        chat_client=LiteLlmResponsesClient(),
        instructions="You are a helpful assistant.",
...
@pytest.mark.flaky
@skip_if_lite_llm_integration_tests_disabled
async def test_lite_llm_responses_client_agent_hosted_mcp_tool() -> None:
    """Integration test for HostedMCPTool with LiteLLM Response Agent using Microsoft Learn MCP."""
