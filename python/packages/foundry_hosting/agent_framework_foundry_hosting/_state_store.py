...
class FoundryCheckpointStore:
    ...
    ``allowed_checkpoint_types`` parameter using ``"module:qualname"`` format. This mirrors the
    behavior of ``FileCheckpointStorage`` and ``CosmosCheckpointStorage``, ensuring the
    ``RestrictedUnpickler`` allowlist is applied on every hosted checkpoint load rather than
    falling back to unrestricted ``pickle.loads``.
    """
...
class CheckpointStoreProvider(ContextScopedStoreProvider[CheckpointStorage]):
    ...
    Checkpoint deserialization is restricted to the built-in safe type set plus framework and
    OpenAI SDK types by default. Applications that persist custom state types can widen the
    allowlist by passing ``allowed_checkpoint_types`` (``"module:qualname"`` strings), which is
    threaded to every ``FoundryCheckpointStore`` this provider creates.
    """
    ...
    def __init__(self, *, allowed_checkpoint_types: list[str] | None = None) -> None:
        ...
        Args:
            allowed_checkpoint_types: Additional types (beyond the built-in safe set
                and framework types) permitted during checkpoint deserialization,
                as ``"module:qualname"`` strings.
        """
