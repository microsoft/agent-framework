...
class _OutputItemTracker:
    ...
    def __init__(self, stream: ResponseEventStream) -> None:
        ...
        # OAuth consent requests seen while converting this response's content. Lives on the
        # tracker because the tracker is created once per response and is already threaded
        # through both the agent and workflow paths, and it is read once the inner handler
        # finishes to pick the terminal event.
        self.consent = _ConsentTracker()
    ...
    async def handle(
        ...
        *,
        ...
        if content.type == "text" and content.text is not None:
            ...
            # client can render a consent prompt instead of an empty assistant turn. Without
            # this branch the content falls through to the `else` below and is dropped with
            # only a warning, leaving the user no way to grant access.
            for event in self._close():
                yield event
            ...
            if consent_link is None:
                ...
                self.consent.dropped.add((content.consent_link or "", server_label))
            else:
                ...
                if consent_key not in self.consent.emitted:
                    self.consent.emitted.add(consent_key)
                    for event in _emit_oauth_consent_item(
                        self._stream,
                        str(self._stream.response["id"]),
                        consent_link,
                        server_label,
                    ):
                        yield event
