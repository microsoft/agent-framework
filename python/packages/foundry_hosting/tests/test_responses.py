...
class TestWorkflowAgentHosting:
    ...
    async def test_conversation_response_checkpoints_support_branching(self, stream: bool) -> None:
        ...
        first = await _post(server, input_text="first", conversation_id="conversation-1", stream=stream)
        second = await _post(server, input_text="second", conversation_id="conversation-1", stream=stream)
        ...
        assert response_body(second)["status"] == "completed"
        assert response_body(branch)["status"] == "completed"
        branch_text = [
            part["text"]
