...
class AgentFunctionApp(DFAppBase):
    ...
    def _register_workflow_routes(self, workflow: Workflow) -> None:
        ...
        async def start_workflow_orchestration(
            ...
            status_url = build_workflow_status_url(base_url, workflow_name, instance_id)

            return func.HttpResponse(
                json.dumps({
                    "instanceId": instance_id,
                    "statusQueryGetUri": status_url,
                    "respondUri": build_workflow_respond_url(base_url, workflow_name, instance_id, "{requestId}"),
                    ...
                    pending_requests: list[dict[str, Any]] = [
                        {
                            ...
                            "respondUrl": build_workflow_respond_url(
                                base_url, workflow_name, instance_id, qualified_id
                            ),
                        }
    ...
    def _build_base_url(self, request_url: str) -> str:
        """Extract the base URL (scheme + host) from a request URL, honoring the route prefix."""
        return strip_route_prefix(request_url)
