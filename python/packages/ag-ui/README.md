...
By default, `AGUIChatClient` and `AGUIHttpService` reuse an internally owned HTTP client
for connection pooling but do not persist response cookies. This applies to all runs,
...
Applications that need cookie-based upstream sessions, including those migrating from
