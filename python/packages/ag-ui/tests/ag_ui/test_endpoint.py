...
async def test_add_endpoint_with_agent_protocol(build_chat_client):
    ...
    app = FastAPI()
    agent = Agent(name="test", instructions="Test agent", chat_client=build_chat_client())
...
async def test_endpoint_event_streaming(build_chat_client):
    ...
    app = FastAPI()
    agent = Agent(name="test", instructions="Test agent", chat_client=build_chat_client("Streamed response"))
...
async def test_endpoint_multiple_paths(build_chat_client):
    ...
    app = FastAPI()
    agent1 = Agent(name="agent1", instructions="First agent", chat_client=build_chat_client("Response 1"))
    agent2 = Agent(name="agent2", instructions="Second agent", chat_client=build_chat_client("Response 2"))
