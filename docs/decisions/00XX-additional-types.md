...
| **Mistral**             | *No dedicated built-in “Computer Use” tool.*                                                                                                                                                                                                         | Mistral’s Agents API provides a **tool system** but does not include a pre-defined GUI control tool in its built-in connectors [\[docs.mistral.ai\]](https://docs.mistral.ai/agents/introduction). Developers can integrate custom tools (via function calling or MCP connectors) for UI automation. In practice, one would define a custom tool (e.g. via MCP server) that can move the mouse, click, etc., but there is no native parameterized “Computer Use” tool in Mistral’s public docs [\[docs.mistral.ai\]](https://docs.mistral.ai/agents/introduction). *(Common parameters like screen size or environment would need to be handled by the custom implementation.)*                                                                                                                                                                                                                              |
...
| **Mistral**            | **Function Call / Tool Entry**                                                                                                                                                                                                                                                                                  | In Mistral’s Agents API, a tool invocation would appear as a **structured entry** in the conversation, not a plain text message. When a Mistral agent uses a tool, the model outputs a **function call** to that tool (via the connectors or function-calling interface). For example, if a custom “UI control” tool were provided, the model would emit a call like `{"tool": "MyUITool", "parameters": {...}}` instead of normal text. The Mistral docs note that *“all actions in a conversation are tracked as structured entries”* [\[datacamp.com\]](https://www.datacamp.com/tutorial/mistral-agents-api), which includes tool calls. (Since Mistral doesn’t have a native GUI tool, this would be a custom function call defined by the developer.)                                                                                                                                                                                                                                                 |
...
| **Mistral**            | **Function Response / Conversation Entry**                                                                                                                                                                                                                                                 | Since Mistral relies on custom tool integration, the developer likewise returns the result in a structured form. If using function-calling, this would mean sending a **function result** object. In the Agents API, tool outputs become part of the conversation state (as entries containing the tool’s output) [\[datacamp.com\]](https://www.datacamp.com/tutorial/mistral-agents-api). For example, if the tool took a screenshot, the developer would attach the image data to the function’s return payload. The format would be JSON (e.g., returning an object with fields for the image or any text result). This mirrors the approach of other providers: the agent loop consumes a structured result rather than treating it as an ordinary message.                                                                                                                                                                                                                                                                                                            |
...
class ComputerResultContent(BaseContent):
    ...
    along with optional metadata like the current URL or error information.
    ...
    Attributes:
        ...
        current_url: The current URL if operating in a browser context.
...
| `current_url`     | N/A                          | N/A                                  | `args.url`                    | Browser URL if applicable                    |
...
| **Mistral**   | **Function-calling interface** (Mistral AI API): Mistral’s docs don’t have a fixed “shell” tool by name, but developers can **define a tool function** for shell execution. Using the Mistral function calling mechanism [\[docs.mistral.ai\]](https://docs.mistral.ai/capabilities/function_calling), [\[mistralai.github.io\]](https://mistralai.github.io/mistral-common/usage/tools/), one could register a function (e.g., `run_shell(command: str)`) with a JSON schema specifying a `command` parameter. The model will produce a `ToolCall` (function call) with the tool name and arguments [\[mistralai.github.io\]](https://mistralai.github.io/mistral-common/usage/tools/). In Mistral’s open-source CLI (Vibe CLI for Devstral), a **built-in terminal tool** is provided – users can prefix inputs with `!` to execute shell commands [\[mistral.ai\]](https://mistral.ai/news/devstral-2-vibe-cli). This indicates Mistral’s ecosystem supports shell execution by exposing a *Terminal/Shell* tool under the hood (with the command string as input), though the exact JSON fields are handled internally. |
...
| **Mistral**   | Mistral’s function calling returns outputs via a **ToolMessage**. After a ToolCall (function call) is made, the developer runs the corresponding tool and produces a ToolMessage containing the result. The ToolMessage includes the same `tool_call_id` and the tool name, plus a `content` field with the function’s return value [\[mistralai.github.io\]](https://mistralai.github.io/mistral-common/usage/tools/), [\[mistralai.github.io\]](https://mistralai.github.io/mistral-common/usage/tools/). In a shell command scenario, the function could return the command’s output text (likely as a string). Thus, the return type is effectively a string of the command output, possibly encapsulated in JSON if the function is defined to return structured data. By default, an unconstrained shell tool would just yield text output. Mistral doesn’t enforce a particular format for the shell’s stdout vs stderr – it’s up to the tool implementation. In practice, one might combine them or handle errors via an error message return.                                                                                                                                |
...
| **Mistral**   | Mistral’s function-calling paradigm means the assistant’s message sequence explicitly separates tool use and final answer. The typical flow: the assistant outputs a `<ToolCall>` (function call) message (not directly shown to the user), then the system executes it and returns a `<ToolResult>`/`ToolMessage`, after which the assistant produces a normal message. This final assistant message can include the results in a readable format. Since Mistral’s interface is often implemented by developers (using libraries like `mistral-common`), it’s up to the developer or UI to decide how to display the result. In a CLI like Mistral Vibe or similar agent, the output might be printed directly to console as the result of the `!` command. In a chat interface, the model would likely present the shell output in a markdown block or as plain text. The important point is that **all providers, including Mistral, aim to present shell outputs in a user-friendly way** – usually mirroring how a terminal would display it (verbatim text). There isn’t a unique Mistral-specific format beyond that.                                                                                                                                                                                                                                                                                                                             |
