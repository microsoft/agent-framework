...
- Neutral: app code using the supported host shape can be short.
- Bad: the framework owns concerns already handled by web frameworks and protocol SDKs.
...
- parsing protocol-native input into Agent Framework run input, options, and an optional session key;
...
- protocol-specific session-key helper functions when useful, such as `telegram_session_key(chat_id)`;
...
Session continuity remains explicit. Protocol helpers may return or expose a session key derived from protocol-native
data, such as:

- OpenAI Responses `previous_response_id` or a newly minted response id;
- Telegram chat id;
- Activity conversation id;
- host-platform isolation context.
...
```python
session = await state.session_store.get(session_key)
...
`SessionStore.reset(session_key)` rotates or clears the current session for non-persisted servers. Persisted stores can
implement the same async interface later.

The session key is a partition key, not proof of identity. App or platform code must authenticate and authorize any
...
from agent_framework_hosting_responses import create_response_id, responses_from_run, responses_to_run
...
async def responses(body: dict = Body(...), x_api_key: str | None = Header(default=None)) -> JSONResponse:
    ...
    response_id = create_response_id(previous_response_id=run.get("previous_response_id"))
    ...
    result = await state.target.run(
        run["messages"],
        session=await state.session_store.get(run["session_key"]),
        options=options,
    )
...
class ResponsesView(View):
    def post(self, request: HttpRequest) -> JsonResponse:
        ...
        result = async_to_sync(state.target.run)(
            run["messages"],
            session=async_to_sync(state.session_store.get)(run["session_key"]),
            options=options,
        )
        return JsonResponse(responses_from_run(result, response_id=response_id))
```
