...
| AWS (Strands) | **Approach 3** Returns an [AgentResult](https://github.com/strands-agents/sdk-python/blob/main/src/strands/agent/agent_result.py) (Primary) with messages and a reason for the run's completion. | **Approach 2** [Streams events](https://github.com/strands-agents/sdk-python/blob/main/src/strands/agent/agent.py) (Primary+Secondary) including, response text, current_tool_use, even data from "callbacks" (strands plugins) |
...
| AWS (Strands) | **Approach 2** Supports a special invocation method called [structured_output](https://github.com/strands-agents/sdk-python/blob/main/src/strands/agent/agent.py) |
...
| AWS (Strands) | Exposes a [stop_reason](https://github.com/strands-agents/sdk-python/blob/main/src/strands/types/event_loop.py) property on the [AgentResult](https://github.com/strands-agents/sdk-python/blob/main/src/strands/agent/agent_result.py) class with options that are tied closely to LLM operations. |
