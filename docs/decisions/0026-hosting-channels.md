...
- **`ChannelResponseHook`** — the *outbound* counterpart to `ChannelRunHook`. Applied per destination after target invocation, before the channel pushes the result onto its wire. Receives the `HostedRunResult` and a `ChannelResponseContext` (request, channel name, destination identity, originating flag, `is_echo` phase flag) and returns a (possibly transformed) `HostedRunResult`. Used for channel-side projections of multi-modal output: a text-only wire flattens to `.text`, a card-capable channel renders structured contents, a workflow result with `value=` can be rendered into a channel-friendly text by the host developer. Hooks are stored as a `response_hook` attribute on the channel instance — duck-typed, not part of the `Channel` Protocol, so adding hook support to a new channel package never breaks the Protocol contract. The host clones `HostedRunResult` per destination before invoking the hook so one channel's transform cannot leak into another's payload.
- **Multi-modality is preserved end-to-end.** `HostedRunResult.messages` carries the full ordered list of `Message` objects produced by the agent (or workflow). The host never collapses to text. Channels — through their `response_hook` and their native serializer — decide what subset their wire can carry.
