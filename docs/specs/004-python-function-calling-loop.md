...
| Invocations durable function history | A later invocation restores one completed call/result pair across host recreation without re-executing the tool; both streaming and non-streaming responses preserve the final text and model-bound conversation history. | `packages/foundry_hosting/tests/test_invocations.py::TestSessionLifecycle::test_function_history_is_restored_without_reexecuting_completed_tool` |
