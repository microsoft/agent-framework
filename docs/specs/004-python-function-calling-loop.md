...
| Legacy stored approval | A versioned serialized pending request without `function_call.id` retains exact request-id binding once and warns only when resumed. | `test_versioned_legacy_pending_approval_retains_consume_once_compatibility`, `packages/core/tests/core/test_types.py::test_legacy_function_call_deserialization_does_not_generate_an_occurrence_id` |
