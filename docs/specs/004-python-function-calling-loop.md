...
- A provider adapter that knows its protocol invalidated or cancelled local function calls already exposed in response
  content raises `FunctionCallInvalidatedException`. The function invocation layer then abandons only that current
  model/tool iteration, clears its request budget state, restores the last valid service continuation, and re-raises
...
| Invalidated per-service-call persistence | An invalidated service response, its partial calls, and any nonexistent results are not success-persisted, and invalid content does not advance session continuation state. | `packages/core/tests/core/test_function_invocation_logic.py::test_invalidated_service_call_is_not_persisted` |
