...
- `Agent.as_tool()` keeps child function approvals inside the delegated invocation. A child
  ...
  invocation-budget state remain isolated. Parent and child `ToolApprovalMiddleware` instances must use distinct
  `source_id` values; an overlap fails before the child runs.
