...
- `Agent.as_tool()` keeps child function approvals inside the delegated invocation. A child
  ...
  Interactive, delayed, or durable approval belongs in a workflow. When `propagate_session=True`, parent and child
  `ToolApprovalMiddleware` instances must use distinct `source_id` values; an overlap fails before the child runs.
