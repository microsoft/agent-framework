...
- An ordinary exception raised by function middleware or a tool body becomes one terminal error `function_result`
  ...
  tool result, the in-flight parallel batch is cancelled, no further tool call starts, no further model call is
  made, and the exception propagates to the caller (for streaming runs, when the stream is consumed). Batch
